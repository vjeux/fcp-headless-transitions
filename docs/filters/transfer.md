...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAETransfer` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAETransfer initWithAPIManager:]` stores preset id **6** (`0x6`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAETransfer initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAETransfer`

```asm
000000000011dbe8	sub	sp, sp, #0x20
000000000011dbec	stp	x29, x30, [sp, #0x10]
000000000011dbf0	add	x29, sp, #0x10
000000000011dbf4	adrp	x8, 695 ; 0x3d4000
000000000011dbf8	ldr	x8, [x8, #0xfe0] ; Objc class ref: bad class ref
000000000011dbfc	stp	x0, x8, [sp]
000000000011dc00	adrp	x8, 831 ; 0x45c000
000000000011dc04	ldr	x1, [x8, #0x278]
000000000011dc08	mov	x0, sp
000000000011dc0c	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011dc10	cbz	x0, 0x11dc28
000000000011dc14	adrp	x8, 833 ; 0x45e000
000000000011dc18	add	x8, x8, #0xc0c
000000000011dc1c	ldrsw	x8, [x8]
000000000011dc20	mov	w9, #0x6
000000000011dc24	str	w9, [x0, x8]
000000000011dc28	ldp	x29, x30, [sp, #0x10]
000000000011dc2c	add	sp, sp, #0x20
000000000011dc30	ret
```
