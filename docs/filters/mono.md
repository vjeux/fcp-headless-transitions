...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAEMono` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

### Subclass initialiser — `-[PAEMono initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAEMono`

```asm
000000000011d9f4	sub	sp, sp, #0x20
000000000011d9f8	stp	x29, x30, [sp, #0x10]
000000000011d9fc	add	x29, sp, #0x10
000000000011da00	adrp	x8, 695 ; 0x3d4000
000000000011da04	ldr	x8, [x8, #0xfb0] ; Objc class ref: bad class ref
000000000011da08	stp	x0, x8, [sp]
000000000011da0c	adrp	x8, 831 ; 0x45c000
000000000011da10	ldr	x1, [x8, #0x278]
000000000011da14	mov	x0, sp
000000000011da18	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011da1c	cbz	x0, 0x11da30
000000000011da20	adrp	x8, 833 ; 0x45e000
000000000011da24	add	x8, x8, #0xc0c
000000000011da28	ldrsw	x8, [x8]
000000000011da2c	str	wzr, [x0, x8]
000000000011da30	ldp	x29, x30, [sp, #0x10]
000000000011da34	add	sp, sp, #0x20
000000000011da38	ret
```
