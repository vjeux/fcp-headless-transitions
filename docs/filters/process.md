...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAEProcess` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAEProcess initWithAPIManager:]` stores preset id **5** (`0x5`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAEProcess initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAEProcess`

```asm
000000000011db94	sub	sp, sp, #0x20
000000000011db98	stp	x29, x30, [sp, #0x10]
000000000011db9c	add	x29, sp, #0x10
000000000011dba0	adrp	x8, 695 ; 0x3d4000
000000000011dba4	ldr	x8, [x8, #0xfd8] ; Objc class ref: bad class ref
000000000011dba8	stp	x0, x8, [sp]
000000000011dbac	adrp	x8, 831 ; 0x45c000
000000000011dbb0	ldr	x1, [x8, #0x278]
000000000011dbb4	mov	x0, sp
000000000011dbb8	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011dbbc	cbz	x0, 0x11dbd4
000000000011dbc0	adrp	x8, 833 ; 0x45e000
000000000011dbc4	add	x8, x8, #0xc0c
000000000011dbc8	ldrsw	x8, [x8]
000000000011dbcc	mov	w9, #0x5
000000000011dbd0	str	w9, [x0, x8]
000000000011dbd4	ldp	x29, x30, [sp, #0x10]
000000000011dbd8	add	sp, sp, #0x20
000000000011dbdc	ret
```
