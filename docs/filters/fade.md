...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAEFade` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAEFade initWithAPIManager:]` stores preset id **3** (`0x3`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAEFade initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAEFade`

```asm
000000000011daec	sub	sp, sp, #0x20
000000000011daf0	stp	x29, x30, [sp, #0x10]
000000000011daf4	add	x29, sp, #0x10
000000000011daf8	adrp	x8, 695 ; 0x3d4000
000000000011dafc	ldr	x8, [x8, #0xfc8] ; Objc class ref: bad class ref
000000000011db00	stp	x0, x8, [sp]
000000000011db04	adrp	x8, 831 ; 0x45c000
000000000011db08	ldr	x1, [x8, #0x278]
000000000011db0c	mov	x0, sp
000000000011db10	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011db14	cbz	x0, 0x11db2c
000000000011db18	adrp	x8, 833 ; 0x45e000
000000000011db1c	add	x8, x8, #0xc0c
000000000011db20	ldrsw	x8, [x8]
000000000011db24	mov	w9, #0x3
000000000011db28	str	w9, [x0, x8]
000000000011db2c	ldp	x29, x30, [sp, #0x10]
000000000011db30	add	sp, sp, #0x20
000000000011db34	ret
```
