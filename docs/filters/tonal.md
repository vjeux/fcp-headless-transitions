...
- **PAE class:** `Tonal`
- **Plugin UUID:** `0962EB7B-F32A-42E6-B2B3-C5F4C02AEEAF`
- **Node names in corpus:** Tonal (1)
- **Corpus usage:** 1 files, 1 instances

## What it does

Tonal is a preset tonal/contrast color look. It exposes only Mix; the underlying grade is a canned transform.

> **Note.** Not implemented; a preset look with only Mix exposed. (unverified) exact transform.
...
|---|---|---|---|---|
| Mix | float | 1 | 0 .. 0 | Wet/dry blend of the tonal look, 0-1 continuous. NOT a boolean. |
...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAETonal` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAETonal initWithAPIManager:]` stores preset id **1** (`0x1`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAETonal initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAETonal`

```asm
000000000011da44	sub	sp, sp, #0x20
000000000011da48	stp	x29, x30, [sp, #0x10]
000000000011da4c	add	x29, sp, #0x10
000000000011da50	adrp	x8, 695 ; 0x3d4000
000000000011da54	ldr	x8, [x8, #0xfb8] ; Objc class ref: bad class ref
000000000011da58	stp	x0, x8, [sp]
000000000011da5c	adrp	x8, 831 ; 0x45c000
000000000011da60	ldr	x1, [x8, #0x278]
000000000011da64	mov	x0, sp
000000000011da68	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011da6c	cbz	x0, 0x11da84
000000000011da70	adrp	x8, 833 ; 0x45e000
000000000011da74	add	x8, x8, #0xc0c
000000000011da78	ldrsw	x8, [x8]
000000000011da7c	mov	w9, #0x1
000000000011da80	str	w9, [x0, x8]
000000000011da84	ldp	x29, x30, [sp, #0x10]
000000000011da88	add	sp, sp, #0x20
000000000011da8c	ret
```
