...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAEChrome` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAEChrome initWithAPIManager:]` stores preset id **4** (`0x4`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAEChrome initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAEChrome`

```asm
000000000011db40	sub	sp, sp, #0x20
000000000011db44	stp	x29, x30, [sp, #0x10]
000000000011db48	add	x29, sp, #0x10
000000000011db4c	adrp	x8, 695 ; 0x3d4000
000000000011db50	ldr	x8, [x8, #0xfd0] ; Objc class ref: bad class ref
000000000011db54	stp	x0, x8, [sp]
000000000011db58	adrp	x8, 831 ; 0x45c000
000000000011db5c	ldr	x1, [x8, #0x278]
000000000011db60	mov	x0, sp
000000000011db64	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011db68	cbz	x0, 0x11db80
000000000011db6c	adrp	x8, 833 ; 0x45e000
000000000011db70	add	x8, x8, #0xc0c
000000000011db74	ldrsw	x8, [x8]
000000000011db78	mov	w9, #0x4
000000000011db7c	str	w9, [x0, x8]
000000000011db80	ldp	x29, x30, [sp, #0x10]
000000000011db84	add	sp, sp, #0x20
000000000011db88	ret
```
