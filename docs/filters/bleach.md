...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAEBleach` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAEBleach initWithAPIManager:]` stores preset id **9** (`0x9`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAEBleach initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAEBleach`

```asm
000000000011dc90	sub	sp, sp, #0x20
000000000011dc94	stp	x29, x30, [sp, #0x10]
000000000011dc98	add	x29, sp, #0x10
000000000011dc9c	adrp	x8, 695 ; 0x3d4000
000000000011dca0	ldr	x8, [x8, #0xff0] ; Objc class ref: bad class ref
000000000011dca4	stp	x0, x8, [sp]
000000000011dca8	adrp	x8, 831 ; 0x45c000
000000000011dcac	ldr	x1, [x8, #0x278]
000000000011dcb0	mov	x0, sp
000000000011dcb4	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011dcb8	cbz	x0, 0x11dcd0
000000000011dcbc	adrp	x8, 833 ; 0x45e000
000000000011dcc0	add	x8, x8, #0xc0c
000000000011dcc4	ldrsw	x8, [x8]
000000000011dcc8	mov	w9, #0x9
000000000011dccc	str	w9, [x0, x8]
000000000011dcd0	ldp	x29, x30, [sp, #0x10]
000000000011dcd4	add	sp, sp, #0x20
000000000011dcd8	ret
```
