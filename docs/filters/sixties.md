...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAESixties` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAESixties initWithAPIManager:]` stores preset id **14** (`0xe`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAESixties initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAESixties`

```asm
000000000011dde0	sub	sp, sp, #0x20
000000000011dde4	stp	x29, x30, [sp, #0x10]
000000000011dde8	add	x29, sp, #0x10
000000000011ddec	adrp	x8, 696 ; 0x3d5000
000000000011ddf0	ldr	x8, [x8, #0x10] ; Objc class ref: bad class ref
000000000011ddf4	stp	x0, x8, [sp]
000000000011ddf8	adrp	x8, 831 ; 0x45c000
000000000011ddfc	ldr	x1, [x8, #0x278]
000000000011de00	mov	x0, sp
000000000011de04	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011de08	cbz	x0, 0x11de20
000000000011de0c	adrp	x8, 833 ; 0x45e000
000000000011de10	add	x8, x8, #0xc0c
000000000011de14	ldrsw	x8, [x8]
000000000011de18	mov	w9, #0xe
000000000011de1c	str	w9, [x0, x8]
000000000011de20	ldp	x29, x30, [sp, #0x10]
000000000011de24	add	sp, sp, #0x20
000000000011de28	ret
```
