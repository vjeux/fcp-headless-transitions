...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAECool` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAECool initWithAPIManager:]` stores preset id **28** (`0x1c`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAECool initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAECool`

```asm
000000000011e224	sub	sp, sp, #0x20
000000000011e228	stp	x29, x30, [sp, #0x10]
000000000011e22c	add	x29, sp, #0x10
000000000011e230	adrp	x8, 695 ; 0x3d5000
000000000011e234	ldr	x8, [x8, #0x78] ; Objc class ref: bad class ref
000000000011e238	stp	x0, x8, [sp]
000000000011e23c	adrp	x8, 830 ; 0x45c000
000000000011e240	ldr	x1, [x8, #0x278]
000000000011e244	mov	x0, sp
000000000011e248	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011e24c	cbz	x0, 0x11e264
000000000011e250	adrp	x8, 832 ; 0x45e000
000000000011e254	add	x8, x8, #0xc0c
000000000011e258	ldrsw	x8, [x8]
000000000011e25c	mov	w9, #0x1c
000000000011e260	str	w9, [x0, x8]
000000000011e264	ldp	x29, x30, [sp, #0x10]
000000000011e268	add	sp, sp, #0x20
000000000011e26c	ret
```
