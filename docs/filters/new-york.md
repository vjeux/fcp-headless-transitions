...
This is one of Final Cut Pro's built-in **Photos-style colour presets**. It is NOT a `· KF` template: it is a real `PAENewYork` class that subclasses `PAEPhotosFilters` and applies a baked **3-D colour LUT** (cube file) through the shared base renderer. The code below is **verbatim** from the user's licensed FCP install; nothing is paraphrased.

**Preset selector.** `-[PAENewYork initWithAPIManager:]` stores preset id **13** (`0xd`) into the instance; `PAEPhotosFilters` uses it to pick which LUT to load (`lutBitmapForFilter:` / `LUTFromCache:atPath:`).

### Subclass initialiser — `-[PAENewYork initWithAPIManager:]`
Regenerate: `venv/bin/python3 tools/re/disasm_pae.py --method initWithAPIManager PAENewYork`

```asm
000000000011dd8c	sub	sp, sp, #0x20
000000000011dd90	stp	x29, x30, [sp, #0x10]
000000000011dd94	add	x29, sp, #0x10
000000000011dd98	adrp	x8, 696 ; 0x3d5000
000000000011dd9c	ldr	x8, [x8, #0x8] ; Objc class ref: bad class ref
000000000011dda0	stp	x0, x8, [sp]
000000000011dda4	adrp	x8, 831 ; 0x45c000
000000000011dda8	ldr	x1, [x8, #0x278]
000000000011ddac	mov	x0, sp
000000000011ddb0	bl	0x252308 ; Objc message: _objc_msgSendSuper2
000000000011ddb4	cbz	x0, 0x11ddcc
000000000011ddb8	adrp	x8, 833 ; 0x45e000
000000000011ddbc	add	x8, x8, #0xc0c
000000000011ddc0	ldrsw	x8, [x8]
000000000011ddc4	mov	w9, #0xd
000000000011ddc8	str	w9, [x0, x8]
000000000011ddcc	ldp	x29, x30, [sp, #0x10]
000000000011ddd0	add	sp, sp, #0x20
000000000011ddd4	ret
```
