...
hook=ctypes.CDLL(REPO+"/fct/instrument/bothsq.dylib"); hook.setup_bothsq.restype=ctypes.c_int
print("setup", hook.setup_bothsq(), flush=True)
...
for i,frac in enumerate([0.02,0.1,0.3,0.5]):
    ozengine.render_frame(doc,C.IMG_A,C.IMG_B,frac*2.333,f"/tmp/bothsq_{i}.png")
print("done",flush=True); open("/tmp/bothsq_done.txt","w").write("done")
