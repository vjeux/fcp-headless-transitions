...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/ginf_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static double h_fn(void* self, void* cm, double index){
    ...
    if(cnt<230) L("#%ld index=%.4f -> influence=%.4f\n", cnt, index, r);
    cnt++;
    return r;
}
