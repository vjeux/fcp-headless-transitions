...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/calcidx3_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, char** arr, unsigned int count){
    ...
    if(!done){
        ...
        char* begin=arr[0]; char* end=arr[1];
        long stride=0xf8; long n=(end-begin)/stride;
        for(long i=0;i<n && i<130;i++){
            char* elem=begin+i*stride;
            char* obj=*(char**)(elem+0xe8);
            double ci = obj?*(double*)(obj+0xb0):-9;
            double of = obj?*(double*)(obj+0xa8):-9;
            double x=*(double*)(elem+0x0), y=*(double*)(elem+0x8), z=*(double*)(elem+0x10);
            L("#%ld pos=(%.1f,%.1f,%.1f) of=%.4f calcIdx=%.1f\n", i, x,y,z, of, ci);
        }
    }
}
