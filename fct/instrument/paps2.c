...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/paps2_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, void* cm, char* p, void* vals){
    if(cnt<130 && p){
        ...
        // find position: scan 0x8..0x90 for the two doubles in screen range
        char buf[400]; int n=0;
        for(int off=0x8; off<=0x90; off+=8){
            double v=*(double*)(p+off);
            if(v>-1000&&v<1000&&(v>50||v<-50)) n+=snprintf(buf+n,sizeof(buf)-n,"[%x]=%.0f ",off,v);
        }
        L("#%ld idx(a8)=%.3f b0=%.1f pos: %s\n", cnt, a8,b0, buf);
    }
    cnt++;
    o_fn(self,cm,p,vals);
}
