...
ozengine.render_frame(doc0,C.IMG_A,C.IMG_B,0.5*2.333,"/tmp/pm.png")
hook=ctypes.CDLL(REPO+"/fct/instrument/hookmulti.dylib")
hook.setup_multi.restype=ctypes.c_int; hook.report.restype=None
print("setup_multi rc", hook.setup_multi(), flush=True)
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
for t in [0.1,0.3,0.5,0.7,0.9]:
    ozengine.render_frame(doc,C.IMG_A,C.IMG_B,t,"/tmp/pm.png")
hook.report()
print("renders done",flush=True)
open("/tmp/pm_done.txt","w").write("done")
