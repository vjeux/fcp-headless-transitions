...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gpg_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// genPosGeometry(this x0, double frac d0, CMTime& x1, PCVector3& out x2, PCVector3& x3)
typedef void (*fn_t)(void*, double, void*, double*, double*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, double frac, void* cm, double* out, double* p4){
    o_fn(self,frac,cm,out,p4);
    if(cnt<130 && out) L("gpg frac=%.6f -> x=%.3f y=%.3f z=%.3f\n", frac, out[0],out[1],out[2]);
    cnt++;
}
...
int setup_gpg(void){
    void* ad=dlsym((void*)-2,"_ZN9PSEmitter14genPosGeometryEdRK6CMTimeR9PCVector3IdES5_");
    if(!ad){L("NOSYM\n");return -2;}
