...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/calccount_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef void (*fn_t)(void*, char**, unsigned int);
static fn_t o_fn=NULL; static int call=0;
static void h_fn(void* self, char** arr, unsigned int count){
    o_fn(self,arr,count);
    // log the first 14 orderFractions of this call to distinguish replicators
    char* begin=arr[0]; char* end=arr[1]; long n=(end-begin)/0xf8;
    char buf[400]; int bn=0;
    for(long i=0;i<14 && i<n;i++){ char* obj=*(char**)(begin+i*0xf8+0xe8); double of=obj?*(double*)(obj+0xa8):-9; bn+=snprintf(buf+bn,sizeof(buf)-bn,"%.2f ",of); }
    L("CALL #%d n=%ld this=%p first14of: %s\n", call, n, self, buf);
    call++;
}
