...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/rtile_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// renderImageParticleTile(this x0, HGTile* x1, Sprite& x2)
typedef void (*fn_t)(void*, void*, char*);
static fn_t o_fn=NULL; static long cnt=0; static int frame=-1;
void rtile_frame(int f){ frame=f; }
static void h_fn(void* self, void* tile, char* sp){
    if(frame>=0 && cnt<4000 && sp){
        // Sprite+0x8: position (2 doubles?). +0x100..0x128: color (rgba). Read pos + alpha.
        double px=*(double*)(sp+0x8), py=*(double*)(sp+0x10);
        double c0=*(double*)(sp+0x100), c1=*(double*)(sp+0x108), c2=*(double*)(sp+0x110), c3=*(double*)(sp+0x118);
        L("f%d pos=(%.0f,%.0f) col=(%.3f,%.3f,%.3f,%.3f)\n", frame, px,py, c0,c1,c2,c3);
        cnt++;
    }
    o_fn(self,tile,sp);
}
...
int setup_rtile(void){
    void* ad=dlsym((void*)-2,"_ZN12PSHeliumNode23renderImageParticleTileEP6HGTileRKNS_6SpriteE");
    if(!ad){L("NOSYM\n");return -2;}
