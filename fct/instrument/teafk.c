...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/teafk_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef double (*fn_t)(unsigned int, unsigned int, unsigned int);
static fn_t o_fn=NULL; static long cnt=0; static uint32_t* kbase=NULL;
static double h_fn(unsigned int w0, unsigned int w1, unsigned int w2){
    double r=o_fn(w0,w1,w2);
    if(cnt<8 && kbase) L("teafk(%u,%u,%u)=%.8f keys=[%#x,%#x,%#x]\n", w0,w1,w2, r, kbase[0],kbase[1],kbase[2]);
    cnt++;
    return r;
}
...
int setup_teafk(void){
    void* base=dlsym((void*)-2,"_ZN9PSEmitter12shuffleOrderEjjj"); if(!base){L("NOBASE\n");return -3;}
    kbase=(uint32_t*)((char*)base - 0x1bc78 + 0xe1c60);
    void* ad=(void*)((char*)base - 0x1bc78 + 0x15600);
    ...
    int rc=patch(ad,pt,16); L("setup rc=%d kbase=%p\n",rc,kbase); return rc;
}
