...
static void h_fn(void* self, void* cm, void* particle, void* vals){
    // particle+0xb0 = the per-cell index (double) fed to normalizeIndex. Also dump position fields to ID the cell.
    if(cnt<250 && particle){
        double idx=*(double*)((char*)particle+0xb0);
        // scan offsets 0x00..0xa0 for position-like doubles (screen coords ~ -1000..1000)
        if(cnt<4){ char buf[900]; int o=0; o+=snprintf(buf,900,"#%ld idx=%.1f:",cnt,idx);
          for(int off=0;off<0xb0;off+=8){ double v=*(double*)((char*)particle+off); if(v!=0.0) o+=snprintf(buf+o,900-o," [%x]=%.1f",off,v);} 
          L("%s\n",buf); }
    }
    cnt++;
