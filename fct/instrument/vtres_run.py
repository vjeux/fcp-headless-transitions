...
hook=ctypes.CDLL(REPO+"/fct/instrument/vtres.dylib"); hook.setup_vtres.restype=ctypes.c_int
print("setup", hook.setup_vtres(), flush=True)
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
# render at frac 0.3 (where scramble is clearest)
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/vtr.png")
print("done",flush=True); open("/tmp/vtr_done.txt","w").write("done")
