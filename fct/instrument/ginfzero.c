...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/ginfzero_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static double h_fn(void* self, void* cm, double idx){
    ...
    double enc = 0.0; if(enc<0)enc=0; if(enc>1)enc=1;
