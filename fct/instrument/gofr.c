...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gofr_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// genOrderFilledRect(this x0, double x d0, double y d1, uint w2, uint w3, CMTime x4, double* out x5, double* x6)
typedef void (*fn_t)(void*, double, double, unsigned int, unsigned int, void*, double*, double*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, double x, double y, unsigned int w2, unsigned int w3, void* cm, double* out, double* o2){
    o_fn(self,x,y,w2,w3,cm,out,o2);
    if(cnt<200) L("gofr x=%.5f y=%.5f w2=%u w3=%u -> of=%.6f\n", x,y,w2,w3, out?*out:-9);
    cnt++;
}
...
int setup_gofr(void){
    void* ad=dlsym((void*)-2,"_ZN9PSEmitter18genOrderFilledRectEddjjRK6CMTimePdS3_");
    if(!ad){L("NOSYM\n");return -2;}
