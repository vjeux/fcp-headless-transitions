...
hook=ctypes.CDLL(REPO+"/fct/instrument/gpfr.dylib"); hook.setup_gpfr.restype=ctypes.c_int
print("setup", hook.setup_gpfr(), flush=True)
...
for i,frac in enumerate([0.02,0.1,0.3,0.5]):
    ozengine.render_frame(doc,C.IMG_A,C.IMG_B,frac*2.333,f"/tmp/gpfr_{i}.png")
print("done",flush=True); open("/tmp/gpfr_done.txt","w").write("done")
