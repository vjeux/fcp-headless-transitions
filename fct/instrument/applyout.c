...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/applyout_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, void* cm, void* particle, void* vals){
    double b0 = particle? *(double*)((char*)particle+0xb0):-9;
    unsigned int e0 = particle? *(unsigned int*)((char*)particle+0xe0):0;
    o_fn(self,cm,particle,vals);
    // read output completion/opacity at values+0x58
    double outc = vals? *(double*)((char*)vals+0x58):-9;
    // also dump a few nearby fields to find opacity
    double v50 = vals? *(double*)((char*)vals+0x50):-9;
    double v60 = vals? *(double*)((char*)vals+0x60):-9;
    if(cnt<260) L("#%ld e0=%u b0=%.1f out58=%.5f v50=%.5f v60=%.5f\n", cnt, e0, b0, outc, v50, v60);
    cnt++;
}
