...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/aasb_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, void* cm, char* p, void* vals){
    if(cnt<130 && p){
        ...
        // scan offsets for position (values in [-950,950])
        char buf[512]; int n=0;
        for(int off=0x08; off<=0x60; off+=8){
            double v=*(double*)(p+off);
            if(v>-1000 && v<1000 && (v>1||v<-1)) n+=snprintf(buf+n,sizeof(buf)-n,"[%#x]=%.1f ",off,v);
        }
        L("#%ld a8=%.4f b0=%.1f pos: %s\n", cnt, a8, b0, buf);
    }
    cnt++;
    o_fn(self,cm,p,vals);
}
