...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/pcap_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// PSParticleType::applyAllSequenceBehaviors(this x0, CMTime& x1, PSParticle* x2, PSSequencedValues& x3)
typedef void (*fn_t)(void*, void*, void*, void*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, void* cmtime, void* particle, void* vals){
    // particle+0xb0 = index (double). particle position: dump several doubles near start.
    if(cnt<130 && particle){
        double* p=(double*)particle;
        double idx=*(double*)((char*)particle+0xb0);
        double* q=(double*)((char*)particle+0x40);
        L("P#%ld idx=%.3f @40:%.1f %.1f %.1f %.1f %.1f %.1f @70:%.1f %.1f %.1f %.1f\n",
          cnt, idx, q[0],q[1],q[2],q[3],q[4],q[5], *(double*)((char*)particle+0x70),*(double*)((char*)particle+0x78),*(double*)((char*)particle+0x80),*(double*)((char*)particle+0x88));
    }
    cnt++;
    o_fn(self,cmtime,particle,vals);
}
...
int setup_pcap(void){
    ...
    int rc=patch(ad,pt,16); L("setup_pcap rc=%d\n",rc); return rc;
}
