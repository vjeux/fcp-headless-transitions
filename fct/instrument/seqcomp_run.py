...
hook=ctypes.CDLL(REPO+"/fct/instrument/seqcomp.dylib"); hook.setup_seqcomp.restype=ctypes.c_int
print("setup", hook.setup_seqcomp(), flush=True)  # active=1 from start
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/sc.png")
print("done",flush=True); open("/tmp/sc_done.txt","w").write("done")
