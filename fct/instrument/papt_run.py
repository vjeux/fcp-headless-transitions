...
hook=ctypes.CDLL(REPO+"/fct/instrument/papt.dylib"); hook.setup_papt.restype=ctypes.c_int
print("setup", hook.setup_papt(), flush=True)
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
# render at frac 0.3 (where scramble is clearest)
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/pt.png")
print("done",flush=True); open("/tmp/pt_done.txt","w").write("done")
