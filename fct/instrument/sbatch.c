...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/sbatch_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// TextureSpriteBatcher::add(this x0, Sprite& x1)
typedef void (*fn_t)(void*, char*);
static fn_t o_fn=NULL; static long cnt=0; static int frame=-1;
void sbatch_frame(int f){ frame=f; }
static void h_fn(void* self, char* sp){
...
int setup_sbatch(void){
    void* ad=dlsym((void*)-2,"_ZN12PSHeliumNode20TextureSpriteBatcher3addERKNS_6SpriteE");
    if(!ad){L("NOSYM\n");return -2;}
