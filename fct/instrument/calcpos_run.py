...
hook=ctypes.CDLL(REPO+"/fct/instrument/calcpos.dylib"); hook.setup_calcpos.restype=ctypes.c_int
print("setup", hook.setup_calcpos(), flush=True)
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/cp.png")
print("done",flush=True); open("/tmp/cp_done.txt","w").write("done")
