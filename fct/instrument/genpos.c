...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/genpos_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// genPosGeometry(this x0, double d0, CMTime& x1, PCVector3<double>& out x2, PCVector3<double>& x3)
typedef void (*fn_t)(void*, double, void*, double*, double*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, double d0, void* cm, double* out, double* o2){
    o_fn(self,d0,cm,out,o2);
    if(cnt<160 && out) L("genpos in=%.5f -> x=%.3f y=%.3f z=%.3f\n", d0, out[0],out[1],out[2]);
    cnt++;
}
