...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/shuforder_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef unsigned int (*so_t)(void*, unsigned int, unsigned int, unsigned int);
static so_t g_orig=NULL;
static unsigned int hk(void* self, unsigned int index, unsigned int N, unsigned int seed){
    unsigned int r=g_orig(self,index,N,seed);
    L("SO self=%p index=%u N=%u seed=%u -> %u\n", self, index, N, seed, r);
    return r;
}
...
// resolve the symbol ourselves (RTLD_DEFAULT) so there's no cross-language pointer passing.
int setup_shuf_self(void){
    void* addr = dlsym((void*)-2, "_ZN9PSEmitter12shuffleOrderEjjj");
    L("setup_shuf_self resolved addr=%p\n", addr);
    if(!addr){ L("dlsym FAILED\n"); return -2; }
    uint32_t*t=(uint32_t*)addr; void*tr=mktr(t); if(!tr){L("tramp fail\n");return -1;} g_orig=(so_t)tr;
    uint32_t pt[4]; pt[0]=0x58000051; pt[1]=0xD61F0220; *(uint64_t*)&pt[2]=(uint64_t)hk;
    int rc=patch(t,pt,16); L("patch rc=%d\n",rc); return rc;
}
