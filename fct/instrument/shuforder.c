...
// PSEmitter::shuffleOrder(this x0, uint idx w1, uint count w2, uint seed w3) -> uint w0
typedef unsigned int (*fn_t)(void*, unsigned int, unsigned int, unsigned int);
static fn_t o_fn=NULL; static long cnt=0;
static unsigned int h_fn(void* self, unsigned int idx, unsigned int count, unsigned int seed){
    unsigned int r=o_fn(self,idx,count,seed);
    if(cnt<200) L("shuf idx=%u count=%u seed=%u -> %u\n", idx,count,seed,r);
    cnt++;
    return r;
}
