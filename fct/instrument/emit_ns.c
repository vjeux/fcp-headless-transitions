...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/emit_ns_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// initPropertiesFromShape(this x0, uint w1, CMTime x2, PType x3, PSParticle* x4, OZSimStateElement& x5, PCVector3& x6, bool& x7)
...
static void h_fn(void* x0,unsigned int w1,void* x2,void* x3,void* particle,void* x5,void* vec3,void* x7){
    ...
    if(cnt<130){
        double ph=particle?*(double*)((char*)particle+0xa8):-9;
        double rk=particle?*(double*)((char*)particle+0xb0):-9;
        // PCVector3<double> = 3 doubles (x,y,z) = emission position
        double px=vec3?((double*)vec3)[0]:-9, py=vec3?((double*)vec3)[1]:-9, pz=vec3?((double*)vec3)[2]:-9;
        L("#%ld emitIdx=%u pos=(%.1f,%.1f,%.1f) phase=%.5f rank=%.1f\n", cnt, w1, px,py,pz, ph, rk);
    }
    cnt++;
}
