...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/calcord2_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, void* arr, unsigned int count){
    ...
    if(cnt==0){
        ...
        for(long i=0;i<n && i<130;i++){
            char* e=begin + i*0xf8;
            // elem+0xe8 is a pointer to the particle object; orderFrac at obj+0xa8, index at obj+0xb0
            char* obj=*(char**)(e+0xe8);
            double of=-9, idx=-9;
            if(obj){ of=*(double*)(obj+0xa8); idx=*(double*)(obj+0xb0); }
            L("#%ld obj=%p orderFrac=%.6f index=%.1f\n", i, obj, of, idx);
        }
    }
    cnt++;
}
