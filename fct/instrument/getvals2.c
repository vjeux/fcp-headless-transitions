...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/getvals2_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef void (*fn_t)(void*, void*, double, void*);
static fn_t o_fn=NULL; static int active=0;
static double frac=0;
static double seen[64]; static int seencnt=0;
void gv2_mark(double f){ frac=f; active=1; seencnt=0; L("=== FRAME %.3f ===\n",f); }
static void h_fn(void* self, void* cm, double index, void* vals){
    o_fn(self,cm,index,vals);
    if(active){
        int ii=(int)(index+0.5);
        // log first occurrence of each index this frame
        int dup=0; for(int i=0;i<seencnt;i++) if((int)seen[i]==ii){dup=1;break;}
        if(!dup && seencnt<64){ seen[seencnt++]=ii; double v58=*(double*)((char*)vals+0x58); L("idx=%d v58=%.5f\n", ii, v58); }
    }
}
