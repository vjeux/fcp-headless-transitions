...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/getval_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// getValues(this x0, CMTime& x1, double index d0, PSSequencedValues& x2)
...
static void h_fn(void* self, void* cm, double index, void* vals){
    ...
    if(cnt<260 && vals){
        char* v=(char*)vals;
        L("#%ld idx=%.2f v40=%.4f v48=%.4f v50=%.4f v58=%.4f v60=%.4f v68=%.4f\n", cnt, index,
          *(double*)(v+0x40),*(double*)(v+0x48),*(double*)(v+0x50),*(double*)(v+0x58),*(double*)(v+0x60),*(double*)(v+0x68));
    }
    cnt++;
}
