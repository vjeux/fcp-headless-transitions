...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/ginf2_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef double (*fn_t)(void*, void*, double);
static fn_t o_fn=NULL; static int frame=-1; static int seen[64];
void ginf2_frame(int f){ frame=f; for(int i=0;i<64;i++)seen[i]=0; L("=== FRAME %d ===\n",f); }
static double h_fn(void* self, void* cm, double idx){
    double r=o_fn(self,cm,idx);
    int ii=(int)(idx+0.5);
    if(frame>=0 && ii>=0 && ii<64 && !seen[ii]){ seen[ii]=1; L("idx=%d infl=%.4f\n", ii, r); }
    return r;
}
