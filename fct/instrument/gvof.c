...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gvof_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// PSSequenceBehavior::getValues(this x0, CMTime& x1, double orderFrac d0, PSSequencedValues& x2)
typedef void (*fn_t)(void*, void*, double, void*);
static fn_t o_fn=NULL; static long cnt=0; static int frame=0;
void gvof_frame(int f){ frame=f; }
static void h_fn(void* self, void* cm, double ofrac, void* vals){
    o_fn(self,cm,ofrac,vals);
    if(cnt<250){ double op=*(double*)((char*)vals+0x58); L("f%d gv of=%.5f -> op=%.5f\n", frame, ofrac, op); }
    cnt++;
}
