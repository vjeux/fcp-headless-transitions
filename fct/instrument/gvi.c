...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gvi_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// OZChannel::getValueAsInt(this x0, CMTime& x1, double d0) const -> int (w0)
typedef int (*fn_t)(void*, void*, double);
static fn_t o_fn=NULL; static long cnt=0; static int active=0;
static int h_fn(void* self, void* cm, double d){
    int r=o_fn(self,cm,d);
    // self is an OZChannel; log its address (low bits identify which channel by offset from emitter) + value.
    if(active && cnt<4000){ L("gvi self=%p -> %d\n", self, r); cnt++; }
    return r;
}
...
void gvi_activate(int a){ active=a; }
int setup_gvi(void){
    void* ad=dlsym((void*)-2,"_ZNK9OZChannel13getValueAsIntERK6CMTimed");
    if(!ad){L("NOSYM\n");return -2;}
