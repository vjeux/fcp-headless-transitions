...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gcomp_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// getCompletion(this x0, CMTime& x1, double index d0) -> double
...
static double h_fn(void* self, void* cm, double index){
    ...
    if(active) L("idx=%.1f -> comp=%.5f\n", index, r);
    return r;
}
void gcomp_mark(const char* s){ L("=== %s ===\n", s); active=1; }
