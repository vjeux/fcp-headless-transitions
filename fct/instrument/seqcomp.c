...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/seqcomp_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// getSequenceCompletion(this x0, CMTime& x1, double index d0, bool& x2, double& x3)
typedef void (*fn_t)(void*, void*, double, void*, double*);
static fn_t o_fn=NULL; static long cnt=0; static int active=1;
static void h_fn(void* self, void* cm, double index, void* b, double* outc){
    o_fn(self,cm,index,b,outc);
    if(active && cnt<400){ L("seqComp idx=%.3f -> %.6f\n", index, outc?*outc:-9); cnt++; }
}
void seqcomp_mark(){ active=1; }
...
int setup_seqcomp(void){
    void* ad=dlsym((void*)-2,"_ZN14PSParticleType21getSequenceCompletionERK6CMTimedRbRd");
    if(!ad){L("NOSYM\n");return -2;}
