...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/getcomp_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// getInfluence(this x0, CMTime& x1, double index d0) -> double completion
typedef double (*fn_t)(void*, void*, double);
static fn_t o_inf=NULL; static long cnt=0;
static double h_inf(void* self, void* cm, double index){
    double r=o_inf(self,cm,index);
    if(1) L("INF idx=%.4f -> %.6f\n", index, r);
    cnt++;
    return r;
}
...
int setup_getcomp(void){
    void* ad=dlsym((void*)-2,"_ZN18PSSequenceBehavior12getInfluenceERK6CMTimed");
    if(!ad){L("NOSYM inf\n");return -2;}
    void* tr=mktr((uint32_t*)ad); if(!tr)return -1; o_inf=(fn_t)tr;
    uint32_t pt[4]; pt[0]=0x58000051; pt[1]=0xD61F0220; *(uint64_t*)&pt[2]=(uint64_t)h_inf;
    int rc=patch(ad,pt,16); L("setup inf rc=%d\n",rc); return rc;
}
