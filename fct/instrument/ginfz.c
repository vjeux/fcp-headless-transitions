...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/ginfz_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef double (*fn_t)(void*, void*, double);
static fn_t o_fn=NULL; static double forced=0.0; static int mode=0;
void ginfz_mode(int m){ mode=m; } // 0=return 0 (content baseline), 1=return idx/27
static double h_fn(void* self, void* cm, double idx){
    o_fn(self,cm,idx);
    if(mode==0) return 0.0;
    double e=idx/27.0; if(e<0)e=0; if(e>1)e=1; return e;
}
