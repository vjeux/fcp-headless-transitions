...
hook=ctypes.CDLL(REPO+"/fct/instrument/getval.dylib"); hook.setup_getval.restype=ctypes.c_int
print("setup", hook.setup_getval(), flush=True)  # active=1 from start
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/gv.png")
print("done",flush=True); open("/tmp/gv_done.txt","w").write("done")
