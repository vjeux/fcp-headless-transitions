...
hook=ctypes.CDLL(REPO+"/fct/instrument/calcord2.dylib"); hook.setup_calcord2.restype=ctypes.c_int
print("setup", hook.setup_calcord2(), flush=True)
doc=ozengine.load_doc(REPO+"/fct/minimized/Objects__Squares/case.motr")
ozengine.render_frame(doc,C.IMG_A,C.IMG_B,0.3*2.333,"/tmp/co.png")
print("done",flush=True); open("/tmp/co_done.txt","w").write("done")
