...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/gpfr_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// genPosFilledRect(this x0, double w d0, double h d1, CMTime& x1, PCVector3& out x2)
typedef void (*fn_t)(void*, double, double, void*, double*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, double w, double h, void* cm, double* out){
    o_fn(self,w,h,cm,out);
    if(cnt<160 && out) L("gpfr w=%.4f h=%.4f -> x=%.3f y=%.3f z=%.3f\n", w,h, out[0],out[1],out[2]);
    cnt++;
}
...
int setup_gpfr(void){
    void* ad=dlsym((void*)-2,"_ZN9PSEmitter16genPosFilledRectEddRK6CMTimeR9PCVector3IdE");
    if(!ad){L("NOSYM\n");return -2;}
