...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/calcidxe_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
...
static void h_fn(void* self, void* arr, unsigned int w2){
    ...
    if(calls==0){
        ...
        for(long i=0;i<n && i<130;i++){
            ...
            for(int off=0x0; off<=0xf0; off+=8){ double v=*(double*)(elem+off); if(v==v&&v>-1000&&v<1000&&(v>5||v<-5)) bn+=snprintf(buf+bn,sizeof(buf)-bn,"[%#x]=%.0f ",off,v); }
