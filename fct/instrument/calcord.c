...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/calcord_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// calcOrderIndexFromOrderFraction(this x0, OZSimStateArray& x1, uint w2=count)
// After it runs, each state element got its +0xb0 index set from +0xa8 orderFraction.
// state array: x1 -> [begin,end]; each element stride 0xf8; orderFrac at elem+0xa8, index at elem+0xb0.
...
static void h_fn(void* self, void* arr, unsigned int count){
    o_fn(self,arr,count);
    if(cnt==0){
        // arr is OZSimStateArray& : first 2 ptrs = begin,end
        char** pp=(char**)arr;
        char* begin=pp[0]; char* end=pp[1];
        long n=(end-begin)/0xf8;
        L("count=%u n_elems=%ld\n", count, n);
        for(long i=0;i<n && i<130;i++){
            char* e=begin + i*0xf8;
            double of=*(double*)(e+0xa8);
            double idx=*(double*)(e+0xb0);
            L("#%ld orderFrac=%.6f index=%.1f\n", i, of, idx);
        }
    }
    cnt++;
}
