...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/ginfenc_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// getInfluence(this x0, CMTime& x1, double idx d0) -> influence. Override to return idx/27 (encode calcIdx as brightness).
...
static double h_fn(void* self, void* cm, double idx){
    o_fn(self,cm,idx); // call original (side effects) but ignore
    double enc = idx/27.0; if(enc<0)enc=0; if(enc>1)enc=1;
    if(cnt<40) L("idx=%.1f -> enc=%.4f\n", idx, enc);
    cnt++;
    return enc;
}
