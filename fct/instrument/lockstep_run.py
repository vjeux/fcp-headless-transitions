...
hook=ctypes.CDLL(REPO+"/fct/instrument/lockstep.dylib"); hook.setup_lockstep.restype=ctypes.c_int
print("setup", hook.setup_lockstep(), flush=True)
...
for i,frac in enumerate([0.02,0.1,0.3,0.5]):
    ozengine.render_frame(doc,C.IMG_A,C.IMG_B,frac*2.333,f"/tmp/lockstep_{i}.png")
print("done",flush=True); open("/tmp/lockstep_done.txt","w").write("done")
