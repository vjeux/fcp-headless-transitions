...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/lockstep_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
typedef void (*gofr_t)(void*, double, double, unsigned int, unsigned int, void*, double*, double*);
typedef void (*gpfr_t)(void*, double, double, void*, double*);
static gofr_t o_gofr=NULL; static gpfr_t o_gpfr=NULL;
static long k_gofr=0, k_gpfr=0;
static void h_gofr(void* s,double x,double y,unsigned int w2,unsigned int w3,void* cm,double* out,double* o2){
    ...
    if(k_gofr<120) L("GOFR #%ld gx=%.4f gy=%.4f of=%.5f\n",k_gofr,x,y,out?*out:-9);
    k_gofr++;
}
static void h_gpfr(void* s,double w,double h,void* cm,double* out){
    o_gpfr(s,w,h,cm,out);
    if(k_gpfr<120) L("GPFR #%ld gw=%.4f gh=%.4f -> sx=%.1f sy=%.1f\n",k_gpfr,w,h,out?out[0]:-9,out?out[1]:-9);
    k_gpfr++;
}
...
int setup_lockstep(void){
    int a=hook("_ZN9PSEmitter18genOrderFilledRectEddjjRK6CMTimePdS3_",(void*)h_gofr,(void**)&o_gofr);
    int b=hook("_ZN9PSEmitter16genPosFilledRectEddRK6CMTimeR9PCVector3IdE",(void*)h_gpfr,(void**)&o_gpfr);
    L("setup gofr=%d gpfr=%d\n",a,b); return a|b;
}
