...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/genorder_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// genOrderLinear(this x0, double orderFrac d0, bool reverse w1, uint N w2, CMTime x3, double* outPos x4, double* outIdx x5)
typedef void (*fn_t)(void*, double, unsigned int, unsigned int, void*, double*, double*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, double orderFrac, unsigned int reverse, unsigned int N, void* cm, double* outPos, double* outIdx){
    o_fn(self,orderFrac,reverse,N,cm,outPos,outIdx);
    if(cnt<250) L("#%ld orderFrac=%.6f reverse=%u N=%u -> pos=%.5f idx=%.1f\n",
        cnt, orderFrac, reverse, N, outPos?*outPos:-9, outIdx?*outIdx:-9);
    cnt++;
}
...
int setup_genorder(void){
    void* ad=dlsym((void*)-2,"_ZN9PSEmitter14genOrderLinearEdbjRK6CMTimePdS3_");
    if(!ad){L("NOSYM\n");return -2;}
