...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/initprop_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// PSEmitter::initPropertiesFromShape(this x0, uint emitIdx w1, CMTime& x2, PSParticleType* x3, PSParticle* x4, ...)
typedef void (*fn_t)(void*,unsigned int,void*,void*,void*,void*,void*);
static fn_t o_fn=NULL; static long cnt=0;
static void h_fn(void* self, unsigned int emitIdx, void* cm, void* ptype, void* particle, void* sim, void* vec){
    o_fn(self,emitIdx,cm,ptype,particle,sim,vec);
    // after the call, the emitter's [+0x8000+0xa8]=phase, [+0xb0]=rank were set; and particle+0xb0=rank(idx)
    double rank_p = particle ? *(double*)((char*)particle+0xb0) : -1;
    if(cnt<200) L("initProp emitIdx=%u particle+0xb0(rank)=%.3f\n", emitIdx, rank_p);
    cnt++;
}
...
int setup_initprop(void){
    ...
    int rc=patch(ad,pt,16); L("setup_initprop rc=%d\n",rc); return rc;
}
