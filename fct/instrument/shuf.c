...
static FILE* g=NULL;
static void L(const char*f,...){ if(!g)g=fopen("/tmp/shuf_trace.txt","a"); va_list a; va_start(a,f); vfprintf(g,f,a); va_end(a); fflush(g);}
// PSEmitter::shuffleOrder(this x0, uint index w1, uint N w2, uint seed w3) -> uint w0
typedef unsigned int (*fn_t)(void*,unsigned int,unsigned int,unsigned int);
static fn_t o_fn=NULL; static long cnt=0;
static unsigned int h_fn(void* self, unsigned int index, unsigned int N, unsigned int seed){
    unsigned int r=o_fn(self,index,N,seed);
    if(cnt<200) L("#%ld index=%u N=%u seed=%u -> perm=%u\n", cnt, index, N, seed, r);
    cnt++;
    return r;
}
