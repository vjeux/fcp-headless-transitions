...
## Your isolated worktree (already created for you)
    cd {wt}
It is a git worktree on branch swarm/subsw-{name} off main, with its own FCT_FRAMES_DIR and
FCT_LOCK (set in the shell you inherit). node_modules + venv are symlinked. NEVER cd back to
{REPO} to build/test — stay in your worktree so you never collide with other agents.
...
or `fct baseline`. Your oracle is your subsystem's isolated node tests:

    fct subswarm test {name}          # caps pack (synthetic single-node scenes vs headless FCP)
                                      #   + your minimized real-slug repros (engine-vs-FCP)
    fct subswarm caps {name}          # just the fast synthetic caps
    fct min-score {mins}   # your node-level real-slug repros (99 dB = pixel-match FCP)
...
1. `fct subswarm test {name}` — see which nodes FAIL and by how much.
2. For a FAILing node: `fct subswarm caps {name} <cap-id>` then re-run with the harness's
   `--keep` to eyeball headless vs TS (the runner prints the kept PNG paths). DECODE why FCP
...
3. Fix ONLY your owned file(s). Re-run `fct subswarm test {name}`. A fix must raise the FAILing
   node(s) and must NOT lower any currently-passing node in your pack or any min-score case.
4. Commit each correctness improvement in your worktree with a clear message, then push it to
   origin/main using the swarm push helper:  bash fct/swarm/push_helper.sh subsw-{name} "<msg>"
...
`fct subswarm test {name}` shows the improvement, report: which nodes moved (before→after dB),
what the real FCP algorithm was, the commit hash(es), and what remains.
"""
