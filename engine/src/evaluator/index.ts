...
function buildTransformMatrix(tx: Transform, timeSec: number, retimeProgress: number = 0, hasRetime: boolean = false): Float64Array {
  ...
  // HINGE GUARD (2026-07-24, Movements__Swing): the retime "static-position ramp"
  // heuristic (resolveWithRetime: ramp default(0)→value by retimeProgress for a layer
  // with a retime curve) is CORRECT for a card that slides IN as media plays (Fall), but
  // WRONG for a fixed HINGE. Movements__Swing's swing panels author pos==anchor==-960 (a
  // left-edge hinge) + a rotation, and carry a MEDIA-FRAME retime (value 1→61). Ramping
  // pos & anchor from 0 moves the hinge (-960→-157 at f4) so the door swings about a
  // drifting pivot (reveals from centre, not the hinge edge). A pure hinge - pos==anchor
  // on an axis WITH a nonzero rotation - must stay FIXED: bypass the retime ramp for those
  // axes (read the authored static value directly). Scoped to the hinge case, so a
  ...
  const posX = resolveWithRetime(tx.positionX, timeSec, 0, retimeProgress, ov?.has('posX') || hingeX, hasRetime)
    + (tx.__mpDeltaX ?? 0);
  const posY = resolveWithRetime(tx.positionY, timeSec, 0, retimeProgress, ov?.has('posY') || hingeY, hasRetime)
    + (tx.__mpDeltaY ?? 0);
  const posZ = resolveWithRetime(tx.positionZ, timeSec, 0, retimeProgress, ov?.has('posZ') || hingeZ, hasRetime);
  ...
  // EXCEPT a fixed HINGE (pos==anchor + rotation): the retime ramp would move the pivot, so the
  // hinge axes read the authored static value directly (see HINGE GUARD above).
  const ancX = resolveWithRetime(tx.anchorX, timeSec, 0, retimeProgress, hingeX, hasRetime);
  const ancY = resolveWithRetime(tx.anchorY, timeSec, 0, retimeProgress, hingeY, hasRetime);
  ...
  const ancZ = resolveWithRetime(tx.anchorZ, timeSec, 0, retimeProgress, hingeZ, hasRetime);
