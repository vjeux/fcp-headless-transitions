/** Movements/Fall,Rotate,Scale ground-truth comparison. */
...
const NAME = process.argv[2] || 'Fall';
const MOTR = `/Users/vjeux/random/motion-renderer/examples/PETemplates.localized/Transitions.localized/Movements.localized/${NAME}.localized/${NAME}.motr`;
const GT = path.resolve(import.meta.dirname, `ground-truth/Movements__${NAME}`);
...
const dumpDir = `/tmp/${NAME.toLowerCase()}_engine`;
...
for (let i=0;i<frames.length;i++){
  ...
  if (dump) savePNG(r, `${dumpDir}/frame_${String(i).padStart(4,'0')}.png`);
  const v = psnr(r, gt);
  if (isFinite(v)) { sum+=v; cnt++; }
  worst.push([i, v]);
}
worst.sort((a,b)=>a[1]-b[1]);
console.log(`${NAME}: mean PSNR ${(sum/cnt).toFixed(2)}dB over ${cnt} frames`);
console.log('Worst frames:', worst.slice(0,8).map(([i,v])=>`f${i}=${v.toFixed(1)}`).join(' '));
