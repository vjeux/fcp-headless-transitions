// OZChannelGradientSample_Factory — ProChannel factory singleton for OZChannelGradientSample.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN31OZChannelGradientSample_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x200e. The remaining methods on this factory (C2/D1/D0/
...
// 8-byte word:
//
//   __ZN31OZChannelGradientSample_Factory13_instanceOnceE
...
//       @0x2020 is the standard libc++ fast-path check for "init done".
//
//   __ZN31OZChannelGradientSample_Factory9_instanceE
//     — a `OZChannelGradientSample_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x204b.
//
// -----------------------------------------------------------------------------
...
//         is `jmp __ZNSt3__18__invoke...<...>` at ProChannel @0x6ef86
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation @0x6ef96 calls __ZN31OZChannelGradientSample_FactoryC2Ev
//         (the C2 base ctor) via operator new(0x88) @0x6efa2, whose
...
// -----------------------------------------------------------------------------
//   * __ZN31OZChannelGradientSample_Factory11getInstanceEv
//       — OZChannelGradientSample_Factory::getInstance() @ProChannel 0x200e
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN31OZChannelGradientSample_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x200e  pushq  %rbp                              ; frame prologue
//   +1     movq   %rsp, %rbp
//   0x2012  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x2016  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   0x201d  movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x2020  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   0x2024  je     0x204b                            ; fast path: skip call_once
//   0x2026  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x202a  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x202e  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x2031  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
//                                                    ; (call_once's `void* arg`)
//   0x2035  movq   %rcx, (%rsi)                      ; *arg = &tuple
//                                                    ; (the void* passed to
//                                                    ; __call_once_proxy)
//   0x2038  leaq   _instanceOnce(%rip), %rdi         ; rdi = &_instanceOnce
//   0x203f  leaq   __call_once_proxy<...>(%rip), %rdx; rdx = &proxy_func
//                                                    ; (function pointer)
//   0x2046  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; signature:
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x204b  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x2052  movq   (%rax), %rax                      ; rax = _instance
//                                                    ; (the return value: the
//                                                    ; singleton pointer, or
//                                                    ; NULL if init raised)
//   0x2055  addq   $0x20, %rsp                       ; frame epilogue
//   0x2059  popq   %rbp
//   0x205a  retq
...
/** @ProChannel BSS `__ZN31OZChannelGradientSample_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; 2n/3n = another thread
 *  running init; -1n (0xFFFF_FFFF_FFFF_FFFF) = completed. getInstance
 *  compares this to $-1 @0x2020 as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x200e read-site

/** @ProChannel BSS `__ZN31OZChannelGradientSample_Factory9_instanceE`.
 *  The singleton pointer. Read @0x204b-0x2052 (the return value).
 *  Written by the __call_once_proxy lambda (a separate function at
 *  ProChannel @0x6ef86/@0x6ef96). */
let _instance: OZChannelGradientSample_Factory | null = null; // @ProChannel BSS 0x204b

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x2046 via ProChannel stub
 ...
 * disasm relies on (the fast-path @0x2020 `cmp $-1` check). */
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x2020 fast-path exit)
 ...
 * (ProChannel @0x6ef86). Body is `jmp __invoke<...>` @0x6ef96, which
 * allocates a fresh OZChannelGradientSample_Factory (size 0x88) via `operator new` @0x6efa2
 * and invokes `OZChannelGradientSample_Factory::OZChannelGradientSample_Factory()` (the C2 base ctor, __ZN31OZChannelGradientSample_FactoryC2Ev, currently
 * ledger status = `todo`) @0x6efad; on success it stores the pointer into
 * `_instance` @0x6efb2. Since neither the C2 ctor nor operator
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0x6ef96..0x6efb2 is:
  //   1. rax = operator new(0x88)       @ProChannel 0x6efa2 (imported __Znwm stub 0xace4c)
  //   2. OZChannelGradientSample_Factory::C2(rax)  @ProChannel 0x6efad
  //   3. _instance = rax                 @ProChannel 0x6efb2
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannelGradientSample_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body @ProChannel 0x6ef96 allocates 0x88 bytes " +
      "via operator new @0x6efa2 then invokes " +
      "__ZN31OZChannelGradientSample_FactoryC2Ev @ProChannel 0x6efad (C2 base ctor, " +
      "ledger status: todo) and stores the result into _instance @0x6efb2. " +
      ...
      "std::__call_once at ProChannel 0x2046.",
  );
}
...
/**
 * `OZChannelGradientSample_Factory` — factory singleton for OZChannelGradientSample
 ...
 */
export class OZChannelGradientSample_Factory {
  /**
   * `OZChannelGradientSample_Factory::getInstance()` — @ProChannel 0x200e
   * (__ZN31OZChannelGradientSample_Factory11getInstanceEv).
   *
   ...
   *
   * Note: the stack tuple + captureless-lambda dance @0x2026..@0x2035 is
   ...
   */
  static getInstance(): OZChannelGradientSample_Factory | null {
    // ------------------------------------------------------------
    // @0x200e..@0x2012 — prologue + 0x20-byte local frame.
    // (No TS-visible effect.)
    // @0x2016..@0x201d — rax = _instanceOnce.
    // @0x2020..@0x2024 — if (_instanceOnce == -1) goto fast_path (0x204b).
    // ------------------------------------------------------------
    if (_instanceOnce !== -1n) {
      // ------------------------------------------------------------
      // @0x2026..@0x2035 — set up libc++ tuple<lambda&&> on the stack.
      // (ABI-level, no TS-visible effect — the proxy just needs a
      // stable void* to dispatch through; we pass a null placeholder.)
      // @0x2038 — rdi = &_instanceOnce.
      // @0x203f — rdx = &__call_once_proxy<...lambda...>.
      // @0x2046 — callq std::__call_once (libc++ stub @0xacdc8).
      // ------------------------------------------------------------
      std_call_once(
        {
          get: (): bigint => _instanceOnce, // (mirrors `movq (%rax),%rax` @0x201d read-side)
          set: (v: bigint): void => {
            _instanceOnce = v;
          },
        },
    ...
    // ------------------------------------------------------------
    // @0x204b..@0x2052 — rax = _instance.
    // @0x2055..@0x205a — epilogue + retq.
    // ------------------------------------------------------------
    return _instance;
  }
}
