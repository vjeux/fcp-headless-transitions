...
export class FFPlayerHealthMeter {
  ...
  // ---- fields first observed by calcHeliumAverage() @0xda1f70 ----
  // Additional per-slot ring-buffer columns (60 slots each):
  //   +0x28    heliumVal[i]   f32[]  — primary Helium metric per slot (summed
  //                                    into the windowed averages).
  //   +0x7e4   skipFlagA[i]   u8[]   — per-slot byte gate A; when nonzero the
  //                                    slot's contribution to the primary
  //                                    (weighted) sum is suppressed.
  //   +0x820   skipFlagB[i]   u8[]   — per-slot byte gate B; when nonzero the
  //                                    slot's contribution to the secondary
  //                                    (delta) sum is suppressed.
  //   +0x988   heliumB[i]     f32[]  — secondary metric per slot.
  //   +0xc58   heliumC[i]     f32[]  — tertiary metric per slot (also the
  //                                    minuend of the C-D delta).
  //   +0xd48   heliumD[i]     f32[]  — subtrahend of the C-D delta.
  // (dropped[+0x2f8], numSamples[+0x1888], ringHead[+0x188c] are reused.)
  // Output/state fields:
  //   +0x1b10  liveEditAllowanceLimit1 (reused) — set to the primary average.
  //   +0x1b18  heliumRunningMax f32   — persistent running maximum (read+written).
  //   +0x1b38  heliumSecondary  f32   — secondary (delta) windowed average output.
  //   +0x1b3c  graphExecBudget  (reused) — set to the weighted primary ratio.
  //   +0x1b44  liveEditAllowanceLimit2 (reused) — set to the mean of (heliumB+heliumC).
  heliumVal: number[] = [];
  skipFlagA: number[] = [];
  skipFlagB: number[] = [];
  heliumB: number[] = [];
  ...
  /**
   * FFPlayerHealthMeter::calcHeliumAverage()
   * @0xADDR Flexo 0x0000000000da1f70  (__ZN19FFPlayerHealthMeter17calcHeliumAverageEv)
   *
   * Faithful transcription of the 283-line disassembly
   * (raw-port/re/disasm/Flexo.__ZN19FFPlayerHealthMeter17calcHeliumAverageEv.s).
   *
   * Walks the 60-slot ring backward from `ringHead` over `numSamples` slots and
   * builds, for each slot (subject to two per-slot byte gates skipFlagA/+0x7e4
   * and skipFlagB/+0x820 and the dropped/+0x2f8 in-(0,8) test):
   *   - a WEIGHTED primary sum of heliumVal(+0x28) with age-dependent weights
   *     (3.0 for the newest <5, 2.0 for 5..14, then table[0.5,1.0] for older),
   *   - a plain running window sum of heliumVal (refined toward 5/15/30-slot
   *     window averages exactly like calcDiskHealth),
   *   - a WEIGHTED secondary (delta) sum of (heliumC-heliumD),
   *   - a plain sum of (heliumB+heliumC).
   * Outputs: liveEditAllowanceLimit1(+0x1b10)=windowAvg, a persistent running max
   * heliumRunningMax(+0x1b18), liveEditAllowanceLimit2(+0x1b44)=mean(B+C),
   * graphExecBudget(+0x1b3c)=max(primaryWinAvg/15, weightedPrimary),
   * heliumSecondary(+0x1b38)=weighted delta average.
   *
   * The control flow is IRREDUCIBLE (two interleaved gated accumulator streams
   * with age-windowed weight selection + several shared jmp tails), so it is
   * rendered as a SINGLE function driven by a labeled dispatch loop keyed on the
   * exact block addresses — no invented helpers (spec Rule 6). Every jmp/jCC
   * becomes `blk=<addr>;continue`. f32 ops wrapped in Math.fround; cvtss2sd/
   * cvtsd2ss transitions explicit; signed int->f32 via `|0`. No in-scope callees,
   * no externs.
   *
   * Constants (Flexo __TEXT __const, fat-x86_64 off 0x4000+VA):
   *   3.0 @0x156cd40 · 2.0 @0x156e940 · 15.0(f32) @0x157012c · 5.0d @0x156ca50 ·
   *   15.0d @0x156f9a0 · 30.0d @0x156efd8 · ageTable[0]=0.5,[1]=1.0 @0x156fd60.
   * AT&T dst-src: `ucomiss %src,%dst` => (dst-src); ja=dst>src, jbe=dst<=src.
   */
  calcHeliumAverage(): void {
    const f = Math.fround;
    const cvtsi2ss = (n: number): number => f(n | 0);

    const W3 = f(3.0); // @0x156cd40 (newest-window weight / delta weight)
    const W2 = f(2.0); // @0x156e940 (mid-window weight)
    const C_15f = f(15.0); // @0x157012c (primaryWinAvg divisor)
    const C_5d = 5.0; // @0x156ca50
    const C_15d = 15.0; // @0x156f9a0
    const C_30d = 30.0; // @0x156efd8
    // ageTable @0x156fd60 = [0.5, 1.0]; indexed by (idx < 0x1e ? 1 : 0).
    const AGE_TABLE = [f(0.5), f(1.0)];
    const ageWeight = (idx: number): number =>
      idx > 0xe ? AGE_TABLE[idx < 0x1e ? 1 : 0] : idx >= 5 ? W2 : W3;

    const RING_LAST = 0x3b; // ring size-1 = 59.

    // Guard: dropped in (0,8) suppresses this slot. (dropped!=0 && dropped<8)
    const droppedSuppresses = (d: number): boolean => d !== 0 && d < 8;

    // ---- registers ----
    let eax = this.numSamples | 0;
    // @0xda1f7b testl %eax,%eax ; je 0xda2012
    if (eax === 0) {
      // L_2012: zero the three primary outputs, then the 0x1b38 tail.
      this.liveEditAllowanceLimit1 = 0; // 0x1b10
      this.liveEditAllowanceLimit2 = 0; // 0x1b44
      this.graphExecBudget = 0; // 0x1b3c
      this.heliumSecondary = 0; // @0xda2418 xorps->movss 0x1b38
      return;
    }

    let ecx = 0; // delta index/count
    let edx = 0; // primary index/count
    let esi = 0; // count used for the sumBC divisor (=numSamples or 1 or 0)
    let r8d = 0; // iteration counter
    let r10 = 0; // current slot index
    let r11d = 0; // ring-prev index scratch

    let xmm0 = 0; // weighted primary sum
    let xmm1 = 0; // primary weight sum
    let xmm2 = 0; // primary window snapshot (fed to /15 at the tail)
    let xmm3 = 0; // weighted delta sum
    let xmm4 = 0; // delta weight sum
    let xmm5 = 0; // sum of (heliumB+heliumC)
    let xmm6 = 0; // window snapshot @<30 / windowAvg carrier
    let xmm7 = 0; // running window sum of heliumVal / final windowAvg
    let xmm8 = 0; // delta running max
    let xmm9 = 0; // scratch (per-slot value)
    let xmm10 = 0; // window snapshot @<15
    let xmm12 = 0; // primary running max
    let xmm13 = 0; // window snapshot @<5
    let xmm14 = 0; // primary running sum (unweighted, snapshot source)

    let dl = false; // (edx >= 15) primary-full-window flag
    let cl = false; // (ecx >= 15) delta-full-window flag

    // @0xda1f83 jle 0xda2035 (eax<0 -> reset)
    let blk: number;
    if (eax < 0) {
      blk = 0x2035;
    } else {
      // ---- first-slot (head) setup @0xda1f89 ----
      const rsi = this.ringHead | 0; // movslq 0x188c
      const xmm3head = f(this.heliumVal[rsi]); // movss 0x28[rsi]
      xmm6 = f(xmm3head + 0); // xmm6 = heliumVal[head]
      // @0xda1fa4 cmpb 0x7e4[rsi] ; jne 0xda1ffd (skipA -> L_1ffd)
      if ((this.skipFlagA[rsi] | 0) === 0) {
        const d = this.dropped[rsi] | 0;
        // @0xda1fd9 testb (dropped<8 & dropped!=0) ; jne 0xda1ffd
        if (!droppedSuppresses(d)) {
          // primary contribution of the head slot (weight W3).
          xmm1 = W3; // movss W3,%xmm1
          xmm2 = f(xmm3head * W3); // xmm2 = head*W3
          xmm0 = f(xmm0 + xmm2); // weightedSum += head*W3
          edx = 1; // primaryCount = 1
          xmm2 = xmm6; // snapshot = heliumVal[head]
          xmm12 = xmm3head; // primary running max = head
        }
      }
      // L_1ffd @0xda1ffd
      // @0xda1ffd cmpb 0x820[rsi] ; je 0xda2062 (B-gate open) else skipB.
      if ((this.skipFlagB[rsi] | 0) !== 0) {
        // skipB: xmm3=xmm4=0 ; goto L_20b1.
        xmm3 = 0;
        xmm4 = 0;
      } else {
        // L_2062 (B-gate open)
        ...
        xmm4 = 0;
        if (!droppedSuppresses(d)) {
          // xmm8 = heliumC[rsi]-heliumD[rsi] ; weighted by W3.
          xmm8 = f(f(this.heliumC[rsi]) - f(this.heliumD[rsi]));
          xmm4 = W3;
          xmm3 = f(xmm3 + f(xmm8 * W3));
          ecx = 1; // deltaCount = 1
        }
      }
      // L_20b1 @0xda20b1
      xmm7 = f(f(this.heliumB[rsi]) + f(this.heliumC[rsi])); // heliumB+heliumC
      xmm5 = f(0 + xmm7); // sumBC = xmm7
      // @0xda20ca cmpl $1,%eax ; jne 0xda20f8
      if (eax === 1) {
        // single-slot finish @0xda20cf.
        xmm7 = f(xmm6 / cvtsi2ss(eax)); // avg = sumHeliumVal / 1
        esi = 1;
        ecx = 0;
        edx = 0;
        dl = false;
        cl = false;
        // ucomiss [0x1b18],xmm7 ; ja 0xda23d1 else 0xda23d9.
        blk = f(this.heliumRunningMax) < xmm7 ? 0x23d1 : 0x23d9;
      } else {
        // ---- MAIN LOOP setup @0xda20f8 ----
        r11d = rsi !== 0 ? rsi - 1 : RING_LAST; // ring-prev with wrap
        r8d = 1;
        xmm14 = xmm2; // primary running sum seed
        xmm10 = xmm6; // window@<15 seed
        xmm13 = xmm6; // window@<5 seed
        xmm7 = xmm6; // running window sum seed
        blk = 0x2166; // enter Ltop
      }
    }
    ...
    dispatch: while (true) {
      switch (blk) {
        case 0x2166: {
          // Ltop: fetch slot r11d.
          r10 = r11d; // movslq %r11d,%r10
          xmm9 = f(this.heliumVal[r10]); // movss 0x28[r10]
          xmm7 = f(xmm7 + xmm9); // running window sum += slot
          // window snapshotting on r8d.
          if (r8d > 0x1d) {
            blk = 0x219e;
            continue dispatch;
          }
          if (r8d > 0xe) {
            // @0xda2220 movaps %xmm7,%xmm6 ; then A-gate test.
            xmm6 = xmm7;
            // cmpb 0x7e4[r10] ; je 0xda21ad else jmp 0xda2267
            blk = (this.skipFlagA[r10] | 0) === 0 ? 0x21ad : 0x2267;
            continue dispatch;
          }
          // r8d <= 14
          let xmm11 = xmm7; // movaps %xmm7,%xmm11
          if (!(r8d < 5)) {
            xmm11 = xmm13; // @0xda218f (>=5)
          }
          xmm6 = xmm7; // @0xda2193
          xmm10 = xmm7;
          xmm13 = xmm11;
        ...
        case 0x219e: {
          // A-gate test for this slot.
          // @0xda219e cmpb 0x7e4[r10] ; jne 0xda2267
          if ((this.skipFlagA[r10] | 0) !== 0) {
            blk = 0x2267;
            continue dispatch;
          }
          blk = 0x21ad;
        ...
        case 0x21ad: {
          // @0xda21ad movl 0x2f8[r10],%r11d ; suppress if dropped in (0,8).
          const d = this.dropped[r10] | 0;
          if (droppedSuppresses(d)) {
            blk = 0x2267;
            continue dispatch;
          }
          // primary accumulate.
          xmm14 = f(xmm14 + xmm9); // primarySum += slot
          // weight by age index edx: >14 -> table; else 2.0(>=5) / 3.0(<5).
          const w = ageWeight(edx);
          // Snapshot xmm2 semantics: for edx<5 (older seeding) xmm2 becomes slot
          //   (movaps %xmm9,%xmm2 @0xda21e8) then reassigned to primarySum after;
          //   for edx>=5 it is set to primarySum (movaps %xmm14,%xmm2). The final
          //   value that reaches the tail's /15 is xmm14 (primarySum) whenever
          //   edx>0, and slot only on the very first (edx==0) primary sample.
          if (edx > 0xe) {
            // @0xda2234/0xda2244: weighted add with table weight; snapshot stays.
            xmm0 = f(xmm0 + f(xmm9 * w));
            xmm1 = f(xmm1 + w);
            // runningMax update.
            if (!(xmm12 > xmm9)) xmm9 = xmm12;
            edx = (edx + 1) | 0;
            xmm12 = xmm9;
            blk = 0x2267;
            continue dispatch;
          ...
          if (edx >= 5) {
            // @0xda21d6..0xda2203: xmm2=primarySum; w=2.0; weighted add; max; edx++.
            xmm2 = xmm14;
            xmm0 = f(xmm0 + f(xmm9 * w));
            xmm1 = f(xmm1 + w);
            if (!(xmm12 > xmm9)) xmm9 = xmm12;
          ...
          // edx < 5 (@0xda21e8): xmm2=slot; w=3.0; weighted add; xmm2=primarySum.
          xmm0 = f(xmm0 + f(xmm9 * w));
          xmm1 = f(xmm1 + w);
          xmm2 = xmm14; // @0xda2203 movaps %xmm14,%xmm2
          // @0xda2207 testl %edx,%edx ; jne 0xda2257 ; else 0xda220b xmm2=xmm14 ; 0xda2261.
          // (both paths leave xmm2=xmm14 here; edx==0 path skips the max test.)
          if (edx !== 0) {
            if (!(xmm12 > xmm9)) xmm9 = xmm12; // @0xda2257 max test
          }
          edx = (edx + 1) | 0; // @0xda2261 incl %edx
          xmm12 = xmm9; // movaps %xmm9,%xmm12
          blk = 0x2267;
          continue dispatch;
        }

        case 0x2267: {
          // B-gate test for this slot.
          // @0xda2267 cmpb 0x820[r10] ; jne 0xda2136
          if ((this.skipFlagB[r10] | 0) !== 0) {
            blk = 0x2136;
            continue dispatch;
          }
          const d = this.dropped[r10] | 0;
          ...
          // xmm15 = heliumC[r10]-heliumD[r10].
          let xmm15 = f(f(this.heliumC[r10]) - f(this.heliumD[r10]));
          const w = ageWeight(ecx);
          if (ecx > 0xe) {
            // @0xda22ec/0xda22fc: table weight.
            xmm9 = f(xmm15 * w);
            xmm3 = f(xmm3 + xmm9);
            xmm4 = f(xmm4 + w);
            if (!(xmm8 > xmm15)) xmm15 = xmm8;
            xmm8 = xmm15; // delta running max carried at advance (movaps in 0x2130)
            // fallthrough advance handled below via 0x2130 semantics.
            blk = 0x2130;
            // stash xmm15 into a temp mirrored by xmm8 already; advance reads xmm8.
            continue dispatch;
          }
          if (ecx >= 5) {
            // @0xda22fc: w=2.0.
          ...
          // ecx < 5 (@0xda22bc): w=3.0.
          xmm9 = f(xmm15 * w);
          xmm3 = f(xmm3 + xmm9);
          xmm4 = f(xmm4 + w);
          // @0xda22d8 testl %ecx,%ecx ; je 0xda2130 (skip max on first)
          if (ecx !== 0) {
            if (!(xmm8 > xmm15)) xmm15 = xmm8; // @0xda22e0 max
          }
          xmm8 = xmm15; // advance @0x2130 does movaps %xmm15,%xmm8
          blk = 0x2130;
          continue dispatch;
        }

        case 0x2130: {
          // delta advance: incl %ecx ; xmm8=xmm15(done above) ; then 0x2136.
          ecx = (ecx + 1) | 0;
          blk = 0x2136;
          continue dispatch;
        }
        ...
        case 0x2136: {
          // ADVANCE: incl %r8d ; sumBC += heliumB[r10]+heliumC[r10] ; ring-prev.
          r8d = (r8d + 1) | 0;
          xmm9 = f(f(this.heliumB[r10]) + f(this.heliumC[r10]));
          xmm5 = f(xmm5 + xmm9);
          r11d = r10 !== 0 ? r10 - 1 : RING_LAST;
          // @0xda215d cmpl %r8d,%eax ; je 0xda2322 (loop done)
          if (eax === r8d) {
            blk = 0x2322;
            continue dispatch;
          }
        ...
        case 0x2322: {
          ...
          dl = edx >= 0xf; // setge
          cl = ecx >= 0xf;
          xmm7 = f(xmm7 / cvtsi2ss(eax)); // windowAvg = sumHeliumVal/numSamples
          // @0xda233c cmpl $5,%eax ; jb 0xda23c6
          if ((eax >>> 0) >= 5) {
            // refine toward window@<5 (xmm13/5).
            {
              const cand = xmm13 / C_5d;
              if (!(cand > xmm7)) xmm7 = f(cand);
            }
            // @0xda236f cmpl $0xf,%eax ; jb 0xda23c6
            if ((eax >>> 0) >= 0xf) {
              const cand = xmm10 / C_15d;
              if (!(cand > xmm7)) xmm7 = f(cand);
              // @0xda239e cmpl $0x1e,%eax ; jb 0xda23c6
              if ((eax >>> 0) >= 0x1e) {
                const cand2 = xmm6 / C_30d;
                if (!(cand2 > xmm7)) xmm7 = f(cand2);
              }
            }
          }
          esi = eax; // @0xda23c6 movl %eax,%esi
          blk = f(this.heliumRunningMax) < xmm7 ? 0x23d1 : 0x23d9;
          continue dispatch;
        }

        case 0x2035: {
          // neg reset: everything zero; esi=edx=ecx=0; xmm7=0.
          xmm5 = 0;
          xmm0 = 0;
          xmm1 = 0;
          xmm2 = 0;
          ...
          dl = false;
          cl = false;
          // @0xda2050 ucomiss [0x1b18],xmm7(0) ; ja 0xda23d1 else 0xda23d9.
          blk = f(this.heliumRunningMax) < 0 ? 0x23d1 : 0x23d9;
          continue dispatch;
        }

        // ===================== shared output tail =====================
        case 0x23d1: {
          // @0xda23d1 movss %xmm7,0x1b18 : heliumRunningMax = windowAvg.
          this.heliumRunningMax = xmm7;
          blk = 0x23d9;
          continue dispatch;
        }

        case 0x23d9: {
          // @0xda23d9 movss %xmm7,0x1b10 : liveEditAllowanceLimit1 = windowAvg.
          this.liveEditAllowanceLimit1 = xmm7;
          // @0xda23e1 testl %eax,%eax ; jle 0xda2435
          if (eax > 0) {
            // @0xda23e8 cvtsi2ss %esi ; divss ; movss 0x1b44.
            this.liveEditAllowanceLimit2 = f(xmm5 / cvtsi2ss(esi));
            // @0xda23f8 testb %dl,%dl ; je 0xda2414
            if (dl) {
              // @0xda23fc weightedPrimary=xmm0/xmm1 ; xmm2/15.0 ; max ; movss 0x1b3c.
              this.graphExecBudget = f(Math.max(f(xmm2 / C_15f), f(xmm0 / xmm1)));
            }
          } else {
            // @0xda2435 xmm5=0 ; movss 0x1b44.
            this.liveEditAllowanceLimit2 = 0;
            // @0xda2440 testb %dl,%dl ; jne 0xda23fc else jmp 0xda2414.
            if (dl) {
              this.graphExecBudget = f(Math.max(f(xmm2 / C_15f), f(xmm0 / xmm1)));
            }
          }
          // @0xda2414 testb %cl,%cl ; jne 0xda2426 ; else 0xda2418.
          if (cl) {
            // @0xda2426 heliumSecondary = xmm3/xmm4.
            this.heliumSecondary = f(xmm3 / xmm4);
          } else {
            // @0xda2418 xorps %xmm3 ; movss %xmm3,0x1b38.
            this.heliumSecondary = 0;
          }
          return; // @0xda2425 / @0xda2434 retq
        }

        default:
          throw new Error(
            "FFPlayerHealthMeter.calcHeliumAverage @Flexo 0xda1f70: unhandled block 0x" +
              blk.toString(16),
          );
      }
    }
  ...
  /**
   * FFPlayerHealthMeter::GetDecodeAheadLevelForDisplay()
   * @0xADDR Flexo 0x0000000000da3920  (__ZN19FFPlayerHealthMeter29GetDecodeAheadLevelForDisplayEv)
   *
   * Trivial single-precision field getter. Returns `this->avgFrameNum2`
   * (the field at +0x1b58, which calcDiskHealth writes at @0xda265c as the
   * per-window average of the secondary frame number) as a float32.
   *
   * FULL DISASM (raw-port/re/disasm/Flexo.__ZN19FFPlayerHealthMeter29GetDecodeAheadLevelForDisplayEv.s — 7 lines):
   *   0xda3920  pushq  %rbp                    ; frame prologue
   *   0xda3921  movq   %rsp, %rbp
   *   0xda3924  movss  0x1b58(%rdi), %xmm0      ; xmm0 = (f32) this->avgFrameNum2
   *   0xda392c  popq   %rbp                     ; frame epilogue
   *   0xda392d  retq                            ; return xmm0
   *   0xda392e  nop                             ; alignment padding (not executed)
   *
   * The `movss` is a single-precision load, so the returned value is wrapped
   * in Math.fround to preserve f32 semantics. Zero in-scope callees, zero
   * externs, no indirect calls — pure field read.
   */
  GetDecodeAheadLevelForDisplay(): number {
