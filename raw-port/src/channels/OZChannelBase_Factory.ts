// OZChannelBase_Factory — ProChannel factory singleton for OZChannelBase.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN21OZChannelBase_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x1786. The remaining methods on this factory (C2/D1/D0/create/
...
// 8-byte word:
//
//   __ZN21OZChannelBase_Factory13_instanceOnceE
...
//       @0x1798 is the standard libc++ fast-path check for "init done".
//
//   __ZN21OZChannelBase_Factory9_instanceE
//     — an `OZChannelBase_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x17c3.
//
// -----------------------------------------------------------------------------
...
//         @0x17be via ProChannel stub 0xacdc8. Same policy as HGMemory's
//         call_once callsites (see raw-port/src/render/HGMemory.ts).
//
//   * __ZNSt3__117__call_once_proxyB9nqe210106<
//         tuple<OZChannelBase_Factory::getInstance()::'lambda'()&&>>(void*)
...
//         is `jmp __ZNSt3__18__invoke...<...>` at ProChannel 0x2da0
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation calls
//         __ZN21OZChannelBase_FactoryC2Ev (the C2 base ctor) via
//         operator new(0x88) at ProChannel 0x2db1, whose ledger status
...
// -----------------------------------------------------------------------------
//   * __ZN21OZChannelBase_Factory11getInstanceEv
//       — OZChannelBase_Factory::getInstance() @ProChannel 0x1786
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN21OZChannelBase_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x1786  pushq  %rbp                              ; frame prologue
//   0x1787  movq   %rsp, %rbp
//   0x178a  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x178e  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   0x1795  movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x1798  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   0x179c  je     0x17c3                            ; fast path: skip call_once
//   0x179e  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x17a2  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x17a6  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x17a9  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
//                                                    ; (call_once's `void* arg`)
//   0x17ad  movq   %rcx, (%rsi)                      ; *arg = &tuple
//                                                    ; (the void* passed to
//                                                    ; __call_once_proxy)
//   0x17b0  leaq   _instanceOnce(%rip), %rdi         ; rdi = &_instanceOnce
//   0x17b7  leaq   __call_once_proxy<...>(%rip), %rdx; rdx = &proxy_func
//                                                    ; (function pointer)
//   0x17be  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; signature:
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x17c3  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x17ca  movq   (%rax), %rax                      ; rax = _instance
//                                                    ; (the return value: the
//                                                    ; singleton pointer, or
//                                                    ; NULL if init raised)
//   0x17cd  addq   $0x20, %rsp                       ; frame epilogue
//   0x17d1  popq   %rbp
//   0x17d2  retq
...
/** @ProChannel BSS `__ZN21OZChannelBase_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; 2n/3n = another thread
 *  running init; -1n (0xFFFF_FFFF_FFFF_FFFF) = completed. getInstance
 *  compares this to $-1 @0x1798 as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x1786 read-site

/** @ProChannel BSS `__ZN21OZChannelBase_Factory9_instanceE`.
 *  The singleton pointer. Read @0x17c3-0x17ca (the return value).
 *  Written by the __call_once_proxy lambda (a separate function at
 *  ProChannel 0x2da0/0x2da5). */
let _instance: OZChannelBase_Factory | null = null; // @ProChannel BSS 0x17c3

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x17be via ProChannel stub
 ...
 * disasm relies on (the fast-path @0x1798 `cmp $-1` check). */
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x1798 fast-path exit)
 ...
 * (ProChannel 0x2da0). Body is `jmp __invoke<...>` @0x2da5, which
 * allocates a fresh OZChannelBase_Factory (size 0x88) via `operator new`
 * @0x2db1 and invokes `OZChannelBase_Factory::OZChannelBase_Factory()`
 * (the C2 base ctor, __ZN21OZChannelBase_FactoryC2Ev, currently ledger
 * status = `todo`) @0x2dbc; on success it stores the pointer into
 * `_instance` @0x2dc8. Since neither the C2 ctor nor operator new are
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0x2da5..0x2dcf is:
  //   1. rax = operator new(0x88)        @ProChannel 0x2db1 (imported __Znwm)
  //   2. OZChannelBase_Factory::C2(rax)  @ProChannel 0x2dbc
  //   3. _instance = rax                 @ProChannel 0x2dc8
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannelBase_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body @ProChannel 0x2da5 allocates 0x88 bytes " +
      "via operator new @0x2db1 then invokes " +
      "__ZN21OZChannelBase_FactoryC2Ev @ProChannel 0x2dbc (C2 base ctor, " +
      "ledger status: todo) and stores the result into _instance @0x2dc8. " +
      ...
      "std::__call_once at ProChannel 0x17be.",
  );
}
...
/**
 * `OZChannelBase_Factory` — factory singleton for OZChannelBase channel
 ...
 * is transcribed here).
 */
export class OZChannelBase_Factory {
  /**
   * `OZChannelBase_Factory::getInstance()` — @ProChannel 0x1786
   * (__ZN21OZChannelBase_Factory11getInstanceEv).
   *
   ...
   *
   * Note: the stack tuple + captureless-lambda dance @0x179e..0x17ad is
   ...
   */
  static getInstance(): OZChannelBase_Factory | null {
    // ------------------------------------------------------------
    // @0x1786..0x178a — prologue + 0x20-byte local frame.
    // (No TS-visible effect.)
    // @0x178e..0x1795 — rax = _instanceOnce.
    // @0x1798..0x179c — if (_instanceOnce == -1) goto fast_path (0x17c3).
    // ------------------------------------------------------------
    if (_instanceOnce !== -1n) {
      // ------------------------------------------------------------
      // @0x179e..0x17ad — set up libc++ tuple<lambda&&> on the stack.
      // (ABI-level, no TS-visible effect — the proxy just needs a
      // stable void* to dispatch through; we pass a null placeholder.)
      // @0x17b0 — rdi = &_instanceOnce.
      // @0x17b7 — rdx = &__call_once_proxy<...lambda...>.
      // @0x17be — callq std::__call_once (libc++ stub @0xacdc8).
      // ------------------------------------------------------------
      std_call_once(
        {
          get: (): bigint => _instanceOnce, // (mirrors `movq (%rax),%rax` @0x1795 read-side)
          set: (v: bigint): void => {
            _instanceOnce = v;
          },
        },
    ...
    // ------------------------------------------------------------
    // @0x17c3..0x17ca — rax = _instance.
    // @0x17cd..0x17d2 — epilogue + retq.
    // ------------------------------------------------------------
    return _instance;
  }
}
