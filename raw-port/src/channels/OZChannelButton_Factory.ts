// OZChannelButton_Factory — ProChannel factory singleton for OZChannelButton.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN23OZChannelButton_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x227e. The remaining methods on this factory are separate ledger
// entries and are OUT OF SCOPE for this file.
//
// -----------------------------------------------------------------------------
// PROCESS-GLOBAL STORAGE
// -----------------------------------------------------------------------------
//   __ZN23OZChannelButton_Factory13_instanceOnceE
//     — the libc++ std::once_flag word. 0 = "not started", -1 (~0UL) =
//       "init done". The `cmpq $-0x1, %rax` at @0x2290 is the standard
//       libc++ fast-path check.
//
//   __ZN23OZChannelButton_Factory9_instanceE
//     — a `OZChannelButton_Factory*` (pointer to the singleton). Written by the
//       __call_once_proxy lambda; read by getInstance @0x22bb.
//
// -----------------------------------------------------------------------------
...
//         @0x22b6 via ProChannel stub 0xacdc8.
//
//   * __ZNSt3__117__call_once_proxy<...tuple<OZChannelButton_Factory::getInstance()::lambda&&>...>
//       — libc++ template instantiation. NOT called by getInstance
//         directly — PASSED AS A DATA REFERENCE to __call_once. The
//         lambda body allocates a OZChannelButton_Factory via operator new and invokes
//         the C2 base ctor (__ZN23OZChannelButton_FactoryC2Ev) — both SEPARATE
...
// -----------------------------------------------------------------------------
//   * __ZN23OZChannelButton_Factory11getInstanceEv
//       — OZChannelButton_Factory::getInstance() @ProChannel 0x227e

/** @ProChannel BSS `__ZN23OZChannelButton_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; -1n = completed.
 *  getInstance compares this to $-1 @0x2290 as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x2286 read-site

/** @ProChannel BSS `__ZN23OZChannelButton_Factory9_instanceE`.
 *  The singleton pointer. Read @0x22bb. Written by the
 *  __call_once_proxy lambda (a SEPARATE ledger entry). */
let _instance: OZChannelButton_Factory | null = null; // @ProChannel BSS 0x22bb

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x22b6 via ProChannel stub
...
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x2290 fast-path exit)
  proxy(arg);
  once.set(-1n);
}
 ...
 * Body allocates a fresh OZChannelButton_Factory via `operator new` and invokes
 * `OZChannelButton_Factory::OZChannelButton_Factory()` (the C2 base ctor,
 * __ZN23OZChannelButton_FactoryC2Ev) then stores the pointer into `_instance`.
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  throw new Error(
    "OZChannelButton_Factory::getInstance() __call_once init lambda not " +
      "yet transcribed — the lambda body allocates a OZChannelButton_Factory " +
      "via operator new (__Znwm, ProChannel stub 0xace4c) and invokes " +
      "__ZN23OZChannelButton_FactoryC2Ev (the C2 base ctor — SEPARATE ledger " +
      "entry) then stores the result into _instance. The proxy is " +
      "invoked from std::__call_once at ProChannel 0x22b6.",
  );
}

/**
 * `OZChannelButton_Factory` — factory singleton. Only its getInstance() accessor is
 * ported in this file; every other method is a separate ledger entry.
 */
export class OZChannelButton_Factory {
  /**
   * `OZChannelButton_Factory::getInstance()` — @ProChannel 0x227e
   * (__ZN23OZChannelButton_Factory11getInstanceEv).
   *
   ...
   */
  static getInstance(): OZChannelButton_Factory | null {
    // @0x227e..prologue.
    // BSS load: rax = _instanceOnce.
    // @0x2290..0x2294 — if (_instanceOnce == -1) goto fast_path (0x22bb).
    if (_instanceOnce !== -1n) {
      // Stack tuple<lambda&&> setup (ABI-level, no TS-visible effect).
      // rdi = &_instanceOnce; rdx = &__call_once_proxy<...>.
      // @0x22b6 — callq std::__call_once (libc++ stub @0xacdc8).
      std_call_once(
        {
    ...
    // @0x22bb — rax = _instance.
    // @0x22ca — epilogue + retq.
    return _instance;
  }
}
