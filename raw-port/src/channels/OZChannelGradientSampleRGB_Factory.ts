// OZChannelGradientSampleRGB_Factory — ProChannel factory singleton for OZChannelGradientSampleRGB.
//
...
// @0x205c. The remaining methods on this factory (C2/D1/D0/
...
//       @0x206e is the standard libc++ fast-path check for "init done".
//
//   __ZN34OZChannelGradientSampleRGB_Factory9_instanceE
//     — a `OZChannelGradientSampleRGB_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x2099.
//
// -----------------------------------------------------------------------------
...
//         @0x2094 via ProChannel stub 0xacdc8. Same policy as
...
//         is `jmp __ZNSt3__18__invoke...<...>` at ProChannel @0x6efd2
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation @0x6efe2 calls
//         __ZN34OZChannelGradientSampleRGB_FactoryC2Ev (the C2 base ctor)
//         via operator new(0x88) @0x6efee, whose ledger status is
//         currently `todo`. It is a TRANSITIVE dependency of getInstance,
//         but not a DIRECT callee — getInstance's disasm only names
//         __call_once as a call target (all other refs are `leaq` data
//         references or memory loads).
//
...
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN34OZChannelGradientSampleRGB_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
...
//   0x2074  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x2078  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x207c  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x207f  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
...
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x205c read-site
 ...
 * disasm relies on (the fast-path @0x206e `cmp $-1` check). */
 ...
 * (ProChannel @0x6efd2). Body is `jmp __invoke<...>` @0x6efe2, which
 * allocates a fresh OZChannelGradientSampleRGB_Factory (size 0x88) via
 * `operator new` @0x6efee and invokes
 * `OZChannelGradientSampleRGB_Factory::OZChannelGradientSampleRGB_Factory()`
 * (the C2 base ctor, __ZN34OZChannelGradientSampleRGB_FactoryC2Ev, currently
 * ledger status = `todo`) @0x6eff9; on success it stores the pointer into
 * `_instance` @0x6effe. Since neither the C2 ctor nor operator
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0x6efe2..0x6effe is:
  //   1. rax = operator new(0x88)       @ProChannel 0x6efee (imported __Znwm stub 0xace4c)
  //   2. OZChannelGradientSampleRGB_Factory::C2(rax)  @ProChannel 0x6eff9
  //   3. _instance = rax                 @ProChannel 0x6effe
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannelGradientSampleRGB_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body @ProChannel 0x6efe2 allocates 0x88 bytes " +
      "via operator new @0x6efee then invokes " +
      "__ZN34OZChannelGradientSampleRGB_FactoryC2Ev @ProChannel 0x6eff9 (C2 base ctor, " +
      "ledger status: todo) and stores the result into _instance @0x6effe. " +
      ...
      "std::__call_once at ProChannel 0x2094.",
  );
}
