// OZChannelObjectRootBase_Factory — ProChannel factory singleton for its target class.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN31OZChannelObjectRootBase_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x21e2. Its shape is identical to every other *_Factory::getInstance
...
// -----------------------------------------------------------------------------
//   __ZN31OZChannelObjectRootBase_Factory13_instanceOnceE
...
//       @0x21f4 is the standard libc++ fast-path check.
//
//   __ZN31OZChannelObjectRootBase_Factory9_instanceE
//     — a `OZChannelObjectRootBase_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x221f.
//
// -----------------------------------------------------------------------------
...
//         @0x221a via ProChannel stub 0xacdc8 (same stub as every
...
//         tuple<OZChannelObjectRootBase_Factory::getInstance()::'lambda'()&&>>(void*)
...
//         `__ZN31OZChannelObjectRootBase_FactoryC2Ev`, then stores the result into `_instance`.
...
// -----------------------------------------------------------------------------
//   * __ZN31OZChannelObjectRootBase_Factory11getInstanceEv
//       — OZChannelObjectRootBase_Factory::getInstance() @ProChannel 0x21e2
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN31OZChannelObjectRootBase_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x21e2  pushq  %rbp                              ; frame prologue
//   ...    movq   %rsp, %rbp
//   0x21e6  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x21ea  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   ...    movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x21f4  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   ...    je     0x221f                            ; fast path: skip call_once
//   0x21fa  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (1-byte
...
//   0x221a  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x221f  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x2226  movq   (%rax), %rax                      ; rax = _instance
//   0x2229  addq   $0x20, %rsp                       ; frame epilogue
//   ...    popq   %rbp
//   0x222e  retq
//   0x222f  nop                                       ; padding
...
/** @ProChannel BSS `__ZN31OZChannelObjectRootBase_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. Compared to $-1 @0x21f4. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x21ea read-site

/** @ProChannel BSS `__ZN31OZChannelObjectRootBase_Factory9_instanceE`.
 *  The singleton pointer. Read @0x221f-0x2226. */
let _instance: OZChannelObjectRootBase_Factory | null = null; // @ProChannel BSS 0x221f

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x221a via ProChannel
...
function std_call_once(
  ...
  if (once.get() === -1n) return; // fast-path (mirrors @0x21f4)
  proxy(arg);
  once.set(-1n);
}
 ...
 * for OZChannelObjectRootBase_Factory::getInstance()::'lambda'(). Body (a separate ledger
 * unit) is `jmp __invoke<...>`, which allocates a fresh OZChannelObjectRootBase_Factory
 * via `operator new` and invokes `__ZN31OZChannelObjectRootBase_FactoryC2Ev` (the C2 base ctor);
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  throw new Error(
    "OZChannelObjectRootBase_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body allocates via operator new " +
      "(__Znwm, libc++ extern) then invokes " +
      "__ZN31OZChannelObjectRootBase_FactoryC2Ev (C2 base ctor, separate " +
      ...
      "invoked from std::__call_once at ProChannel 0x221a.",
  );
}

/**
 * `OZChannelObjectRootBase_Factory` — factory singleton. Only its getInstance()
 * accessor is ported in this file; every other method is a separate
 * ledger entry.
 */
export class OZChannelObjectRootBase_Factory {
  /**
   * `OZChannelObjectRootBase_Factory::getInstance()` — @ProChannel 0x21e2
   * (__ZN31OZChannelObjectRootBase_Factory11getInstanceEv).
   *
   ...
   *
   * The stack tuple @0x21fa.. is an ABI-level artefact of
   ...
   */
  static getInstance(): OZChannelObjectRootBase_Factory | null {
    // @0x21e2..0x21e6: prologue + frame. No TS-visible effect.
    // @0x21ea: rax = _instanceOnce.
    // @0x21f4: if (_instanceOnce == -1) goto 0x221f.
    if (_instanceOnce !== -1n) {
      // @0x21fa..: set up libc++ tuple<lambda&&> on stack (ABI
    ...
    // @0x221f..0x2226: rax = _instance.
    // @0x2229..0x222e: epilogue + retq.
    return _instance;
  }
}
