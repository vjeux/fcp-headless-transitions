// OZChannelScaleOverRange_Factory — ProChannel factory singleton for OZChannelScaleOverRange.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN31OZChannelScaleOverRange_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x23b6. The remaining methods on this factory (C2/D1/D0/
...
// 8-byte word:
//
//   __ZN31OZChannelScaleOverRange_Factory13_instanceOnceE
...
//       @0x23c8 is the standard libc++ fast-path check for "init done".
//
//   __ZN31OZChannelScaleOverRange_Factory9_instanceE
//     — a `OZChannelScaleOverRange_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x23f3.
//
// -----------------------------------------------------------------------------
...
//         is `jmp __ZNSt3__18__invoke...<...>` at ProChannel @0xde01
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation @0xde11 calls __ZN31OZChannelScaleOverRange_FactoryC2Ev
//         (the C2 base ctor) via operator new(0x88) @0xde1d, whose
...
// -----------------------------------------------------------------------------
//   * __ZN31OZChannelScaleOverRange_Factory11getInstanceEv
//       — OZChannelScaleOverRange_Factory::getInstance() @ProChannel 0x23b6
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN31OZChannelScaleOverRange_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x23b6  pushq  %rbp                              ; frame prologue
//   +1     movq   %rsp, %rbp
//   0x23ba  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x23be  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   0x23c5  movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x23c8  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   0x23cc  je     0x23f3                            ; fast path: skip call_once
//   0x23ce  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x23d2  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x23d6  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x23d9  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
//                                                    ; (call_once's `void* arg`)
//   0x23dd  movq   %rcx, (%rsi)                      ; *arg = &tuple
//                                                    ; (the void* passed to
//                                                    ; __call_once_proxy)
//   0x23e0  leaq   _instanceOnce(%rip), %rdi         ; rdi = &_instanceOnce
//   0x23e7  leaq   __call_once_proxy<...>(%rip), %rdx; rdx = &proxy_func
//                                                    ; (function pointer)
//   0x23ee  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; signature:
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x23f3  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x23fa  movq   (%rax), %rax                      ; rax = _instance
//                                                    ; (the return value: the
//                                                    ; singleton pointer, or
//                                                    ; NULL if init raised)
//   0x23fd  addq   $0x20, %rsp                       ; frame epilogue
//   0x2401  popq   %rbp
//   0x2402  retq
...
/** @ProChannel BSS `__ZN31OZChannelScaleOverRange_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; 2n/3n = another thread
 *  running init; -1n (0xFFFF_FFFF_FFFF_FFFF) = completed. getInstance
 *  compares this to $-1 @0x23c8 as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x23b6 read-site

/** @ProChannel BSS `__ZN31OZChannelScaleOverRange_Factory9_instanceE`.
 *  The singleton pointer. Read @0x23f3-0x23fa (the return value).
 *  Written by the __call_once_proxy lambda (a separate function at
 *  ProChannel @0xde01/@0xde11). */
let _instance: OZChannelScaleOverRange_Factory | null = null; // @ProChannel BSS 0x23f3

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x23ee via ProChannel stub
 ...
 * disasm relies on (the fast-path @0x23c8 `cmp $-1` check). */
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x23c8 fast-path exit)
 ...
 * (ProChannel @0xde01). Body is `jmp __invoke<...>` @0xde11, which
 * allocates a fresh OZChannelScaleOverRange_Factory (size 0x88) via `operator new` @0xde1d
 * and invokes `OZChannelScaleOverRange_Factory::OZChannelScaleOverRange_Factory()` (the C2 base ctor, __ZN31OZChannelScaleOverRange_FactoryC2Ev, currently
 * ledger status = `todo`) @0xde28; on success it stores the pointer into
 * `_instance` @0xde34. Since neither the C2 ctor nor operator
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0xde11..0xde34 is:
  //   1. rax = operator new(0x88)       @ProChannel 0xde1d (imported __Znwm stub 0xace4c)
  //   2. OZChannelScaleOverRange_Factory::C2(rax)  @ProChannel 0xde28
  //   3. _instance = rax                 @ProChannel 0xde34
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannelScaleOverRange_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body @ProChannel 0xde11 allocates 0x88 bytes " +
      "via operator new @0xde1d then invokes " +
      "__ZN31OZChannelScaleOverRange_FactoryC2Ev @ProChannel 0xde28 (C2 base ctor, " +
      "ledger status: todo) and stores the result into _instance @0xde34. " +
      ...
      "std::__call_once at ProChannel 0x23ee.",
  );
}
...
/**
 * `OZChannelScaleOverRange_Factory` — factory singleton for OZChannelScaleOverRange
 ...
 */
export class OZChannelScaleOverRange_Factory {
  /**
   * `OZChannelScaleOverRange_Factory::getInstance()` — @ProChannel 0x23b6
   * (__ZN31OZChannelScaleOverRange_Factory11getInstanceEv).
   *
   ...
   *
   * Note: the stack tuple + captureless-lambda dance @0x23ce..@0x23dd is
   ...
   */
  static getInstance(): OZChannelScaleOverRange_Factory | null {
    // ------------------------------------------------------------
    // @0x23b6..@0x23ba — prologue + 0x20-byte local frame.
    // (No TS-visible effect.)
    // @0x23be..@0x23c5 — rax = _instanceOnce.
    // @0x23c8..@0x23cc — if (_instanceOnce == -1) goto fast_path (0x23f3).
    // ------------------------------------------------------------
    if (_instanceOnce !== -1n) {
      // ------------------------------------------------------------
      // @0x23ce..@0x23dd — set up libc++ tuple<lambda&&> on the stack.
      // (ABI-level, no TS-visible effect — the proxy just needs a
      // stable void* to dispatch through; we pass a null placeholder.)
      // @0x23e0 — rdi = &_instanceOnce.
      // @0x23e7 — rdx = &__call_once_proxy<...lambda...>.
      // @0x23ee — callq std::__call_once (libc++ stub @0xacdc8).
      // ------------------------------------------------------------
      std_call_once(
        {
          get: (): bigint => _instanceOnce, // (mirrors `movq (%rax),%rax` @0x23c5 read-side)
          set: (v: bigint): void => {
            _instanceOnce = v;
          },
        },
    ...
    // ------------------------------------------------------------
    // @0x23f3..@0x23fa — rax = _instance.
    // @0x23fd..@0x2402 — epilogue + retq.
    // ------------------------------------------------------------
    return _instance;
  }
}
