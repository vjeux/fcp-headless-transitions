// OZChannel2DOverRange_Factory — ProChannel factory singleton for OZChannel2DOverRange.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN28OZChannel2DOverRange_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x24a0. The remaining methods on this factory (C2/D1/D0/
...
// 8-byte word:
//
//   __ZN28OZChannel2DOverRange_Factory13_instanceOnceE
...
//       @0x24b2 is the standard libc++ fast-path check for "init done".
//
//   __ZN28OZChannel2DOverRange_Factory9_instanceE
//     — a `OZChannel2DOverRange_Factory*` (pointer to the singleton instance).
//       Written by the lambda that std::__call_once invokes on first
//       call; read by getInstance @0x24dd.
//
// -----------------------------------------------------------------------------
...
//         is `jmp __ZNSt3__18__invoke...<...>` at ProChannel @0xe91d
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation @0xe92d calls __ZN28OZChannel2DOverRange_FactoryC2Ev
//         (the C2 base ctor) via operator new(0x88) @0xe939, whose
...
// -----------------------------------------------------------------------------
//   * __ZN28OZChannel2DOverRange_Factory11getInstanceEv
//       — OZChannel2DOverRange_Factory::getInstance() @ProChannel 0x24a0
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN28OZChannel2DOverRange_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x24a0  pushq  %rbp                              ; frame prologue
//   +1     movq   %rsp, %rbp
//   0x24a4  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x24a8  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   0x24af  movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x24b2  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   0x24b6  je     0x24dd                            ; fast path: skip call_once
//   0x24b8  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x24bc  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x24c0  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x24c3  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
//                                                    ; (call_once's `void* arg`)
//   0x24c7  movq   %rcx, (%rsi)                      ; *arg = &tuple
//                                                    ; (the void* passed to
//                                                    ; __call_once_proxy)
//   0x24ca  leaq   _instanceOnce(%rip), %rdi         ; rdi = &_instanceOnce
//   0x24d1  leaq   __call_once_proxy<...>(%rip), %rdx; rdx = &proxy_func
//                                                    ; (function pointer)
//   0x24d8  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; signature:
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x24dd  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x24e4  movq   (%rax), %rax                      ; rax = _instance
//                                                    ; (the return value: the
//                                                    ; singleton pointer, or
//                                                    ; NULL if init raised)
//   0x24e7  addq   $0x20, %rsp                       ; frame epilogue
//   0x24eb  popq   %rbp
//   0x24ec  retq
...
/** @ProChannel BSS `__ZN28OZChannel2DOverRange_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; 2n/3n = another thread
 *  running init; -1n (0xFFFF_FFFF_FFFF_FFFF) = completed. getInstance
 *  compares this to $-1 @0x24b2 as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x24a0 read-site

/** @ProChannel BSS `__ZN28OZChannel2DOverRange_Factory9_instanceE`.
 *  The singleton pointer. Read @0x24dd-0x24e4 (the return value).
 *  Written by the __call_once_proxy lambda (a separate function at
 *  ProChannel @0xe91d/@0xe92d). */
let _instance: OZChannel2DOverRange_Factory | null = null; // @ProChannel BSS 0x24dd

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x24d8 via ProChannel stub
 ...
 * disasm relies on (the fast-path @0x24b2 `cmp $-1` check). */
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x24b2 fast-path exit)
 ...
 * (ProChannel @0xe91d). Body is `jmp __invoke<...>` @0xe92d, which
 * allocates a fresh OZChannel2DOverRange_Factory (size 0x88) via `operator new` @0xe939
 * and invokes `OZChannel2DOverRange_Factory::OZChannel2DOverRange_Factory()` (the C2 base ctor, __ZN28OZChannel2DOverRange_FactoryC2Ev, currently
 * ledger status = `todo`) @0xe944; on success it stores the pointer into
 * `_instance` @0xe950. Since neither the C2 ctor nor operator
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0xe92d..0xe950 is:
  //   1. rax = operator new(0x88)       @ProChannel 0xe939 (imported __Znwm stub 0xace4c)
  //   2. OZChannel2DOverRange_Factory::C2(rax)  @ProChannel 0xe944
  //   3. _instance = rax                 @ProChannel 0xe950
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannel2DOverRange_Factory::getInstance() __call_once init lambda not yet " +
      "transcribed — the lambda body @ProChannel 0xe92d allocates 0x88 bytes " +
      "via operator new @0xe939 then invokes " +
      "__ZN28OZChannel2DOverRange_FactoryC2Ev @ProChannel 0xe944 (C2 base ctor, " +
      "ledger status: todo) and stores the result into _instance @0xe950. " +
      ...
      "std::__call_once at ProChannel 0x24d8.",
  );
}
...
/**
 * `OZChannel2DOverRange_Factory` — factory singleton for OZChannel2DOverRange
 ...
 */
export class OZChannel2DOverRange_Factory {
  /**
   * `OZChannel2DOverRange_Factory::getInstance()` — @ProChannel 0x24a0
   * (__ZN28OZChannel2DOverRange_Factory11getInstanceEv).
   *
   ...
   *
   * Note: the stack tuple + captureless-lambda dance @0x24b8..@0x24c7 is
   ...
   */
  static getInstance(): OZChannel2DOverRange_Factory | null {
    // ------------------------------------------------------------
    // @0x24a0..@0x24a4 — prologue + 0x20-byte local frame.
    // (No TS-visible effect.)
    // @0x24a8..@0x24af — rax = _instanceOnce.
    // @0x24b2..@0x24b6 — if (_instanceOnce == -1) goto fast_path (0x24dd).
    // ------------------------------------------------------------
    if (_instanceOnce !== -1n) {
      // ------------------------------------------------------------
      // @0x24b8..@0x24c7 — set up libc++ tuple<lambda&&> on the stack.
      // (ABI-level, no TS-visible effect — the proxy just needs a
      // stable void* to dispatch through; we pass a null placeholder.)
      // @0x24ca — rdi = &_instanceOnce.
      // @0x24d1 — rdx = &__call_once_proxy<...lambda...>.
      // @0x24d8 — callq std::__call_once (libc++ stub @0xacdc8).
      // ------------------------------------------------------------
      std_call_once(
        {
          get: (): bigint => _instanceOnce, // (mirrors `movq (%rax),%rax` @0x24af read-side)
          set: (v: bigint): void => {
            _instanceOnce = v;
          },
        },
    ...
    // ------------------------------------------------------------
    // @0x24dd..@0x24e4 — rax = _instance.
    // @0x24e7..@0x24ec — epilogue + retq.
    // ------------------------------------------------------------
    return _instance;
  }
}
