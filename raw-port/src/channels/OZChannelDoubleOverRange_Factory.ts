// OZChannelDoubleOverRange_Factory — ProChannel factory singleton for
// OZChannelDoubleOverRange.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN32OZChannelDoubleOverRange_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x2368. The remaining methods on this factory (C2/D1/D0/create/... etc.)
...
// 8-byte word:
//
//   __ZN32OZChannelDoubleOverRange_Factory13_instanceOnceE
...
//       @0x237a is the standard libc++ fast-path check for "init done".
//
//   __ZN32OZChannelDoubleOverRange_Factory9_instanceE
//     — an `OZChannelDoubleOverRange_Factory*` (pointer to the singleton
//       instance). Written by the lambda that std::__call_once invokes
//       on first call; read by getInstance @0x23a5.
//
// -----------------------------------------------------------------------------
...
//         @0x23a0 via ProChannel stub 0xacdc8. Same policy as
...
//         tuple<OZChannelDoubleOverRange_Factory::getInstance()::
//               'lambda'()&&>>(void*)
...
//         is `jmp __ZNSt3__18__invoke<...>` at ProChannel 0x8ef3a
//         (a separate ledger entry NOT in this file's scope). That
//         __invoke instantiation @ProChannel 0x8ef4a calls:
//           - operator new(0x88) @0x8ef56 (__Znwm stub 0xace4c)
//           - OZChannelDoubleOverRange_Factory::C2() @0x8ef61
//             (__ZN32OZChannelDoubleOverRange_FactoryC2Ev — separate
//             ledger entry, currently unported)
//           - stores rax into _instance @0x8ef66
//         Both operator new and the C2 ctor are separate ledger units.
//
...
// -----------------------------------------------------------------------------
//   * __ZN32OZChannelDoubleOverRange_Factory11getInstanceEv
//       — OZChannelDoubleOverRange_Factory::getInstance() @ProChannel 0x2368
//
// -----------------------------------------------------------------------------
// FULL DISASM (raw-port/re/disasm/ProChannel.__ZN32OZChannelDoubleOverRange_Factory11getInstanceEv.s)
// -----------------------------------------------------------------------------
//   0x2368  pushq  %rbp                              ; frame prologue
//   0x2369  movq   %rsp, %rbp
//   0x236c  subq   $0x20, %rsp                       ; 32-byte local frame
//                                                    ; (holds a 3-word libc++
//                                                    ; "tuple<lambda&&>" plus
//                                                    ; alignment padding)
//   0x2370  leaq   _instanceOnce(%rip), %rax         ; rax = &_instanceOnce
//   0x2377  movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x237a  cmpq   $-0x1, %rax                       ; already-init check
//                                                    ; (libc++ writes ~0UL
//                                                    ; on completion)
//   0x237e  je     0x23a5                            ; fast path: skip call_once
//   0x2380  leaq   -0x1(%rbp), %rax                  ; rax = &frame[-1] (a 1-byte
//                                                    ; stack slot — the lambda's
//                                                    ; empty captureless closure
//                                                    ; body; libc++'s tuple<T&&>
//                                                    ; needs a stable address).
//   0x2384  leaq   -0x18(%rbp), %rcx                 ; rcx = &frame[-0x18]
//                                                    ; (the tuple<T&&> slot)
//   0x2388  movq   %rax, (%rcx)                      ; tuple.head = &lambda-slot
//   0x238b  leaq   -0x10(%rbp), %rsi                 ; rsi = &frame[-0x10]
//                                                    ; (call_once's `void* arg`)
//   0x238f  movq   %rcx, (%rsi)                      ; *arg = &tuple
//                                                    ; (the void* passed to
//                                                    ; __call_once_proxy)
//   0x2392  leaq   _instanceOnce(%rip), %rdi         ; rdi = &_instanceOnce
//   0x2399  leaq   __call_once_proxy<...>(%rip), %rdx; rdx = &proxy_func
//                                                    ; (function pointer)
//   0x23a0  callq  std::__call_once                   ; libc++ stub @0xacdc8
//                                                    ; signature:
//                                                    ; (once_flag& = %rdi,
//                                                    ;  void* arg   = %rsi,
//                                                    ;  void(*)(void*) = %rdx)
//   0x23a5  leaq   _instance(%rip), %rax             ; rax = &_instance
//   0x23ac  movq   (%rax), %rax                      ; rax = _instance
//                                                    ; (the return value: the
//                                                    ; singleton pointer, or
//                                                    ; NULL if init raised)
//   0x23af  addq   $0x20, %rsp                       ; frame epilogue
//   0x23b3  popq   %rbp
//   0x23b4  retq
...
/** @ProChannel BSS `__ZN32OZChannelDoubleOverRange_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; 2n/3n = another thread
 *  running init; -1n (0xFFFF_FFFF_FFFF_FFFF) = completed. getInstance
 *  compares this to $-1 @0x237a as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x2370 read-site

/** @ProChannel BSS `__ZN32OZChannelDoubleOverRange_Factory9_instanceE`.
 *  The singleton pointer. Read @0x23a5-0x23ac (the return value).
 *  Written by the __call_once_proxy lambda body @ProChannel 0x8ef66
 *  (inside the __invoke instantiation). */
let _instance: OZChannelDoubleOverRange_Factory | null = null; // @ProChannel BSS 0x23a5

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x23a0 via ProChannel stub
 ...
 * disasm relies on (the fast-path @0x237a `cmp $-1` check). */
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x237a fast-path exit)
 ...
 * (ProChannel 0x8ef3a). Body is `jmp __invoke<...>` @0x8ef45, which
 * (at ProChannel 0x8ef4a) allocates a fresh
 * OZChannelDoubleOverRange_Factory (size 0x88) via `operator new`
 * @0x8ef56 and invokes
 * `OZChannelDoubleOverRange_Factory::OZChannelDoubleOverRange_Factory()`
 * (the C2 base ctor, __ZN32OZChannelDoubleOverRange_FactoryC2Ev,
 * currently ledger status = `todo`) @0x8ef61; on success it stores the
 * pointer into `_instance` @0x8ef66. Since neither the C2 ctor nor
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  // The lambda's body @ProChannel 0x8ef4a..0x8ef71 (inside __invoke):
  //   1. rax = operator new(0x88)        @ProChannel 0x8ef56 (__Znwm stub 0xace4c)
  //   2. OZChannelDoubleOverRange_Factory::C2(rax)
  //                                     @ProChannel 0x8ef61
  //   3. _instance = rax                @ProChannel 0x8ef66
  // C2 is a separate ledger entry (todo). We cite both call sites.
  throw new Error(
    "OZChannelDoubleOverRange_Factory::getInstance() __call_once init " +
      "lambda not yet transcribed — the lambda body @ProChannel 0x8ef4a " +
      "allocates 0x88 bytes via operator new @0x8ef56 then invokes " +
      "__ZN32OZChannelDoubleOverRange_FactoryC2Ev @ProChannel 0x8ef61 " +
      "(C2 base ctor, ledger status: todo) and stores the result into " +
      "_instance @0x8ef66. Neither operator new (__Znwm ProChannel stub " +
      "0xace4c) nor the C2 ctor is yet ported — this lambda function is " +
      "a SEPARATE ledger unit and will be filled in when it is next " +
...
/**
 * `OZChannelDoubleOverRange_Factory` — factory singleton for
 * OZChannelDoubleOverRange channel instances. Only its getInstance()
 ...
 */
export class OZChannelDoubleOverRange_Factory {
  /**
   * `OZChannelDoubleOverRange_Factory::getInstance()` @ProChannel 0x2368
   * (__ZN32OZChannelDoubleOverRange_Factory11getInstanceEv).
   *
   ...
   *
   * Note: the stack tuple + captureless-lambda dance @0x2380..0x238f is
   ...
   */
  static getInstance(): OZChannelDoubleOverRange_Factory | null {
    // ------------------------------------------------------------
    // @0x2368..0x236c — prologue + 0x20-byte local frame.
    // (No TS-visible effect.)
    // @0x2370..0x2377 — rax = _instanceOnce.
    // @0x237a..0x237e — if (_instanceOnce == -1) goto fast_path (0x23a5).
    // ------------------------------------------------------------
    if (_instanceOnce !== -1n) {
      // ------------------------------------------------------------
      // @0x2380..0x238f — set up libc++ tuple<lambda&&> on the stack.
      // (ABI-level, no TS-visible effect — the proxy just needs a
      // stable void* to dispatch through; we pass a null placeholder.)
      // @0x2392 — rdi = &_instanceOnce.
      // @0x2399 — rdx = &__call_once_proxy<...lambda...>.
      // @0x23a0 — callq std::__call_once (libc++ stub @0xacdc8).
      // ------------------------------------------------------------
      std_call_once(
        {
          get: (): bigint => _instanceOnce, // (mirrors `movq (%rax),%rax` @0x2377 read-side)
          set: (v: bigint): void => {
            _instanceOnce = v;
          },
        },
    ...
    // ------------------------------------------------------------
    // @0x23a5..0x23ac — rax = _instance.
    // @0x23af..0x23b4 — epilogue + retq.
    // ------------------------------------------------------------
    return _instance;
  }
}
