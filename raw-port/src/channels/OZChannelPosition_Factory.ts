// OZChannelPosition_Factory — ProChannel factory singleton for OZChannelPosition.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN25OZChannelPosition_Factory11getInstanceEv.s
//
// Only `getInstance()` @ProChannel 0x1c18 is ported here. The remaining
...
// -----------------------------------------------------------------------------
//   __ZN25OZChannelPosition_Factory13_instanceOnceE
//     — libc++ std::once_flag. 0 = not started; -1 = complete. Fast-
//       path @0x1c2a compares to $-1.
//   __ZN25OZChannelPosition_Factory9_instanceE
//     — OZChannelPosition_Factory*. Written by the __call_once_proxy
//       lambda; read by getInstance @0x1c55.
//
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
//   * __ZN25OZChannelPosition_Factory11getInstanceEv
//       — OZChannelPosition_Factory::getInstance() @ProChannel 0x1c18
//
// -----------------------------------------------------------------------------
// FULL DISASM
// -----------------------------------------------------------------------------
//   0x1c18  pushq  %rbp                              ; frame prologue
//   ...     movq   %rsp, %rbp
//   ...     subq   $0x20, %rsp                       ; 32-byte local frame
//   ...     leaq   _instanceOnce(%rip), %rax
//   ...     movq   (%rax), %rax                      ; rax = _instanceOnce
//   0x1c2a  cmpq   $-0x1, %rax                       ; already-init check
//   ...     je     0x1c55                            ; fast path
//   0x1c30  leaq   -0x1(%rbp), %rax                  ; captureless lambda slot
...
//   0x1c42  leaq   _instanceOnce(%rip), %rdi
//   0x1c49  leaq   __call_once_proxy<...>(%rip), %rdx
//   0x1c50  callq  std::__call_once                   ; libc++ stub @0xacdc8
//   0x1c55  leaq   _instance(%rip), %rax
//   0x1c5c  movq   (%rax), %rax                      ; rax = _instance (return)
//   0x1c5f  addq   $0x20, %rsp                       ; epilogue
//   0x1c63  popq   %rbp
//   0x1c64  retq

/** @ProChannel BSS `__ZN25OZChannelPosition_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; -1n = complete.
 *  Fast-path check `cmpq $-1, %rax` @0x1c2a. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x1c18 read-site

/** @ProChannel BSS `__ZN25OZChannelPosition_Factory9_instanceE`.
 *  The singleton pointer. Read @0x1c55-0x1c5c; written by the init lambda. */
let _instance: OZChannelPosition_Factory | null = null; // @ProChannel BSS 0x1c55

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x1c50 via stub 0xacdc8.
...
function std_call_once(
  ...
  if (once.get() === -1n) return; // libc++ fast-path (also @0x1c2a)
  proxy(arg);
  once.set(-1n);
}

/**
 * `__ZNSt3__117__call_once_proxy<...>` for OZChannelPosition_Factory's
 * getInstance lambda. Body @ProChannel 0x76b54 is a 6-byte prologue
 ...
 * is not yet transcribed — cited throwing stub @ProChannel 0x76b54. */
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  throw new Error(
    "OZChannelPosition_Factory::getInstance() __call_once init lambda not " +
      "yet transcribed — the proxy @ProChannel 0x76b54 tail-jumps into " +
      "std::__invoke<lambda()> which allocates the factory via operator " +
      "new (libc++ extern __Znwm 0xace4c), runs the C2 base ctor " +
      "__ZN25OZChannelPosition_FactoryC2Ev (ledger status: todo), and " +
      ...
      "is invoked from std::__call_once at ProChannel 0x1c50.",
  );
}

/**
 * `OZChannelPosition_Factory` — factory singleton. Only getInstance() is
 ...
 */
export class OZChannelPosition_Factory {
  /**
   * `OZChannelPosition_Factory::getInstance()` — @ProChannel 0x1c18
   * (__ZN25OZChannelPosition_Factory11getInstanceEv).
   *
   ...
   *
   *   1. If _instanceOnce == -1, skip to step 3 (@0x1c2a fast-path).
   *   2. Call std::__call_once with the proxy that constructs the
   *      singleton (@0x1c50).
   *   3. Return _instance (@0x1c55).
   */
  static getInstance(): OZChannelPosition_Factory | null {
    // @0x1c18 — prologue + local frame (no TS effect).
    // @0x1c2a — if (_instanceOnce == -1) goto fast_path (0x1c55).
    if (_instanceOnce !== -1n) {
      // @0x1c30 — tuple<lambda&&> stack setup (ABI marshaling; no TS effect).
      // @0x1c42 — rdi = &_instanceOnce.
      // @0x1c49 — rdx = &__call_once_proxy<...>.
      // @0x1c50 — callq std::__call_once (libc++ stub @0xacdc8).
      std_call_once(
        {
    ...
    // @0x1c55..0x1c5c — rax = _instance.
    // @0x1c5f..0x1c64 — epilogue + retq.
    return _instance;
  }
}
