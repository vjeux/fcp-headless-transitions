// OZChannelColorNoAlpha_Factory — ProChannel factory singleton for OZChannelColorNoAlpha.
//
// Transcribed from /Applications/Final Cut Pro.app/Contents/Frameworks/
// ProChannel.framework/Versions/A/ProChannel (x86_64 slice; unadjusted
// VAs from `otool -tV`). Disassembly source:
//   raw-port/re/disasm/ProChannel.__ZN29OZChannelColorNoAlpha_Factory11getInstanceEv.s
//
// This unit ports ONLY the `getInstance()` static singleton accessor at
// @0x1dec. The remaining methods on this factory are separate ledger
// entries and are OUT OF SCOPE for this file.
//
// -----------------------------------------------------------------------------
// PROCESS-GLOBAL STORAGE
// -----------------------------------------------------------------------------
//   __ZN29OZChannelColorNoAlpha_Factory13_instanceOnceE
//     — the libc++ std::once_flag word. 0 = "not started", -1 (~0UL) =
//       "init done". The `cmpq $-0x1, %rax` at @0x1dfe is the standard
//       libc++ fast-path check.
//
//   __ZN29OZChannelColorNoAlpha_Factory9_instanceE
//     — a `OZChannelColorNoAlpha_Factory*` (pointer to the singleton). Written by the
//       __call_once_proxy lambda; read by getInstance @0x1e29.
//
// -----------------------------------------------------------------------------
...
//         @0x1e24 via ProChannel stub 0xacdc8.
//
//   * __ZNSt3__117__call_once_proxy<...tuple<OZChannelColorNoAlpha_Factory::getInstance()::lambda&&>...>
//       — libc++ template instantiation. NOT called by getInstance
//         directly — PASSED AS A DATA REFERENCE to __call_once. The
//         lambda body allocates a OZChannelColorNoAlpha_Factory via operator new and invokes
//         the C2 base ctor (__ZN29OZChannelColorNoAlpha_FactoryC2Ev) — both SEPARATE
...
// -----------------------------------------------------------------------------
//   * __ZN29OZChannelColorNoAlpha_Factory11getInstanceEv
//       — OZChannelColorNoAlpha_Factory::getInstance() @ProChannel 0x1dec

/** @ProChannel BSS `__ZN29OZChannelColorNoAlpha_Factory13_instanceOnceE`.
 *  libc++ std::once_flag word. 0n = not started; -1n = completed.
 *  getInstance compares this to $-1 @0x1dfe as its fast-path check. */
let _instanceOnce: bigint = 0n; // @ProChannel BSS 0x1df4 read-site

/** @ProChannel BSS `__ZN29OZChannelColorNoAlpha_Factory9_instanceE`.
 *  The singleton pointer. Read @0x1e29. Written by the
 *  __call_once_proxy lambda (a SEPARATE ledger entry). */
let _instance: OZChannelColorNoAlpha_Factory | null = null; // @ProChannel BSS 0x1e29

/**
 * `std::__1::__call_once(flag&, void* arg, void(*)(void*))` — libc++
 * (libc++.dylib). Called from getInstance @0x1e24 via ProChannel stub
...
function std_call_once(
  ...
  if (once.get() === -1n) return; // (mirrors 0x1dfe fast-path exit)
  proxy(arg);
  once.set(-1n);
}
 ...
 * Body allocates a fresh OZChannelColorNoAlpha_Factory via `operator new` and invokes
 * `OZChannelColorNoAlpha_Factory::OZChannelColorNoAlpha_Factory()` (the C2 base ctor,
 * __ZN29OZChannelColorNoAlpha_FactoryC2Ev) then stores the pointer into `_instance`.
...
function __call_once_proxy_getInstance_lambda(_arg: unknown): void {
  throw new Error(
    "OZChannelColorNoAlpha_Factory::getInstance() __call_once init lambda not " +
      "yet transcribed — the lambda body allocates a OZChannelColorNoAlpha_Factory " +
      "via operator new (__Znwm, ProChannel stub 0xace4c) and invokes " +
      "__ZN29OZChannelColorNoAlpha_FactoryC2Ev (the C2 base ctor — SEPARATE ledger " +
      "entry) then stores the result into _instance. The proxy is " +
      "invoked from std::__call_once at ProChannel 0x1e24.",
  );
}

/**
 * `OZChannelColorNoAlpha_Factory` — factory singleton. Only its getInstance() accessor is
 * ported in this file; every other method is a separate ledger entry.
 */
export class OZChannelColorNoAlpha_Factory {
  /**
   * `OZChannelColorNoAlpha_Factory::getInstance()` — @ProChannel 0x1dec
   * (__ZN29OZChannelColorNoAlpha_Factory11getInstanceEv).
   *
   ...
   */
  static getInstance(): OZChannelColorNoAlpha_Factory | null {
    // @0x1dec..prologue.
    // BSS load: rax = _instanceOnce.
    // @0x1dfe..0x1e02 — if (_instanceOnce == -1) goto fast_path (0x1e29).
    if (_instanceOnce !== -1n) {
      // Stack tuple<lambda&&> setup (ABI-level, no TS-visible effect).
      // rdi = &_instanceOnce; rdx = &__call_once_proxy<...>.
      // @0x1e24 — callq std::__call_once (libc++ stub @0xacdc8).
      std_call_once(
        {
    ...
    // @0x1e29 — rax = _instance.
    // @0x1e38 — epilogue + retq.
    return _instance;
  }
}
