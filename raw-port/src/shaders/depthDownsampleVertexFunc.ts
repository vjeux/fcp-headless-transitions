// depthDownsampleVertexFunc.ts — trivial pass-through vertex shader used by the
// depth-downsample fragment path.
// @shader depthDownsampleVertexFunc (Lithium)
// Source IR: raw-port/re/shaders/depthDownsampleVertexFunc.ll
// Compiled from: Lithium.framework/Versions/A/Resources/LiSolidShaders.metallib @0x0000000000e589
//
// Byte-identical AIR IR body to sibling vertex passes ccVertexFunc / cdVertexFunc / bcVertexFunc —
// the shaders differ only in metallib entry offset and their name; the body
// (shufflevector + insertvalue pattern) is exactly the same. Kept as a distinct
// ONE-shader-per-file port per SHADERS.md.
//
// LLVM IR signature (from the .ll):
//   define <{ <4 x float>, <2 x float> }>
//     @depthDownsampleVertexFunc(
...
// projection matrix — the CPU-side caller supplies already-clip-space (x, y).
 ...
 */
export interface DepthDownsampleVertexFuncOutput {
  position: [number, number, number, number]; // slot 0 — air.position
  uv: [number, number];                        // slot 1 — air.vertex_output "uv"
}

/**
 * @shader depthDownsampleVertexFunc (Lithium) — faithful port of the AIR IR.
 *
 ...
 */
export function depthDownsampleVertexFunc(
  position: [number, number],
  texCoord: [number, number],
): DepthDownsampleVertexFuncOutput {
