// Faithful transcription @0x000000000b11cd — no shortcut language of any kind.
// @shader soOFlowEstimator::soOFlowEstimator_convolveX (HeliumSenso) @0x000000000b11cd
//
// Provenance: LLVM AIR IR at raw-port/re/shaders/
// soOFlowEstimator__soOFlowEstimator_convolveX.ll, extracted via
// raw-port/tools/shader_disasm.sh from
// HeliumSenso.framework/Versions/A/Resources/default.metallib. The .ll
// header line reads `0x000000000b11cd -- soOFlowEstimator::
// soOFlowEstimator_convolveX:` — that is the shader's entry offset in
...
// per raw-port/army/SHADERS.md (fp32-narrowed accumulate).
//
// This is the X-axis sibling of soOFlowEstimator__soOFlowEstimator_convolveY:
// same struct, same intrinsics, same block layout; only the initial
// sample offset (%24 = <N-1, 0>) and the per-iteration advance
// (%49 = <1, 0>) walk the X axis instead of the Y axis.
//
// Params struct field metadata (!17/!18):
...
//     %20 = 1 - N                                  -- kMin (= -(N-1))
//     %21 = icmp slt (1 - N), N                    -- N >= 1. Loop
//                                                     skipped if N < 1.
//     br i1 %21, label %22, label %26
//
//   block %22 (loop preheader, N >= 1):
//     %23 = N - 1                                  -- kMax
//     %24 = insertelement <undef, 0>, N-1, 0
//                                                  -- <N-1, 0>
//     %25 = coord - <N-1, 0>                       -- initial sample
//                                                     coord: (coord.x
//                                                     - (N-1), coord.y).
//     br label %28
//
...
//                                                     acc (preserved). So
//                                                     only .xy accumulate.
//     %49 = %29 + <1, 0>                           -- advance sample.x by 1.
//     %50 = k + 1
...
/**
 * Params buffer for
 * `soOFlowEstimator::soOFlowEstimator_convolveX` — mirrors the AIR
 * struct at !18 (12-byte packed record, four-byte fields).
 */
export interface SoOFlowEstimatorConvolveXParams {
...
/**
 * Compute kernel `soOFlowEstimator::soOFlowEstimator_convolveX`.
 *
 * Performs a symmetric 1-D convolution along the X axis at every
 ...
 *
 * @shader soOFlowEstimator::soOFlowEstimator_convolveX (HeliumSenso)
 */
export function soOFlowEstimator__soOFlowEstimator_convolveX<TIn, TOut>(
  params: SoOFlowEstimatorConvolveXParams,
  coord: [number, number],
  input: TIn,
  output: TOut,
  ...
  if (runLoop) {
    // %23 = N - 1 = kMax. %24 = <N-1, 0>. %25 = coord - <N-1, 0>
    //   = (coord.x - (N-1), coord.y) = initial sample xy.
    let sampX = (coordX - (N - 1)) | 0;
    let sampY = coordY | 0;
    let k = kMin | 0;
    ...
    while (true) {
      ...
      // %49 = <sampX, sampY> + <1, 0> -- only X advances.
      sampX = (sampX + 1) | 0;
      // sampY unchanged.
