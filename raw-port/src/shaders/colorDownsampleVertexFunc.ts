// colorDownsampleVertexFunc.ts — trivial pass-through vertex shader.
// @shader colorDownsampleVertexFunc (Lithium)
// Source IR: raw-port/re/shaders/colorDownsampleVertexFunc.ll
// Compiled from: Lithium.framework/Versions/A/Resources/LiSolidShaders.metallib @0x000000000f1a9
//
// LLVM IR signature (from the .ll):
//   define <{ <4 x float>, <2 x float> }>
//     @colorDownsampleVertexFunc(
 ...
 */
export interface ColorDownsampleVertexFuncOutput {
  position: [number, number, number, number]; // slot 0 — air.position
  uv: [number, number];                        // slot 1 — air.vertex_output "uv"
}

/**
 * @shader colorDownsampleVertexFunc (Lithium) — faithful port of the AIR IR.
 *
 ...
 */
export function colorDownsampleVertexFunc(
  position: [number, number],
  texCoord: [number, number],
): ColorDownsampleVertexFuncOutput {
