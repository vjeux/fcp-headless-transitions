// Faithful transcription @0x000000000109d9
// @shader mcVertexFunc (Lithium/LiSolidShaders) @0x000000000109d9
// Source IR: raw-port/re/shaders/mcVertexFunc.ll
...
// `0x000000000109d9 -- mcVertexFunc:` — the shader's entry offset in the metallib.
//
// Trivial fullscreen-quad passthrough vertex shader — a member of the vertex-passthrough
// family alongside blurVertexFunc / ccVertexFunc / cdVertexFunc / cocVertexFunc /
// dumpVertexFunc / depthDownsampleVertexFunc. Given a per-vertex 2D clip-space
// `position` and a 2D `texCoord`, emits:
...
// 2/3 of position. The IR body is exactly 4 SSA values:
...
//   air.compile.fast_math_enable   — but there are NO arithmetic ops in this shader, so
//   no Math.fround is required (the 0.0 and 1.0 lane constants are exact fp32).

/**
 * mcVertexFunc — one vertex.
 *
 ...
 *
 * @IR entire function @0x000000000109d9.
 */
export function mcVertexFunc(
