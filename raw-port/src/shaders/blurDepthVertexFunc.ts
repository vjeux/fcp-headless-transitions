// @shader blurDepthVertexFunc (Lithium) @0x0000000000a869
// Source IR: raw-port/re/shaders/blurDepthVertexFunc.ll
// (extracted from Lithium.framework/Versions/A/Resources/LiSolidShaders.metallib)
/**
 * @shader blurDepthVertexFunc (Lithium)
 *
 * Trivial fullscreen-quad passthrough vertex shader. Given a per-vertex 2D
 * clip-space `position` and a 2D `texCoord`, emits:
 *   air.position   = float4(position.x, position.y, 0.0, 1.0)
 *   air.vertex_out = texCoord           (float2, unmodified)
 *
 * Direct TS mapping: no arithmetic, only lane splatting and constant
 * injection at lanes 2/3 of position.
 *
 ...
 *   air.compile.fast_math_enable   — but there are NO arithmetic ops in
 *   this shader, so no Math.fround is required.
 */

/**
 * blurDepthVertexFunc — one vertex.
 *
 ...
 *
 * @IR entire function @0x0000000000a869.
 */
export function blurDepthVertexFunc(
  position: [number, number],
  texCoord: [number, number],
): {
  /** air.position — float4(position.xy, 0.0, 1.0). @IR %4/%5 */
