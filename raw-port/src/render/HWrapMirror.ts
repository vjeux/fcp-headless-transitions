// raw-port/src/render/HWrapMirror.ts
//
// FCP `HWrapMirror` — Ozone render-graph facade for the HgcWrapMirror
// texture-wrap-mirror compositor kernel. HWrapMirror is a thin subclass
// of `HgcWrapMirror` (an HGNode-derived compositor node) that owns a
...
//   raw-port/re/disasm/HWrapMirror.~HWrapMirror.s   @0x470c80 (D1) / @0x470c90 (D0)
//   raw-port/re/disasm/HWrapMirror.GetDOD.s         @0x470cb0
//   raw-port/re/disasm/HWrapMirror.GetROI.s         @0x470d00
//   raw-port/re/disasm/HWrapMirror.setMaskRect.s    @0x470de0
//   raw-port/re/disasm/HWrapMirror.setMatrix.s      @0x470e30
//   raw-port/re/disasm/HWrapMirror.setInvMatrix.s   @0x471000
//
// Ledger addresses (raw-port/army/ledger/Ozone.ledger.json → "HWrapMirror"):
//   0x470c80  HWrapMirror::~HWrapMirror()   [D1 complete dtor — tail-jmp to HgcWrapMirror::~HgcWrapMirror (D2)]
//   0x470c90  HWrapMirror::~HWrapMirror()   [D0 deleting dtor — D2 + HGObject::operator delete]
//   0x470cb0  HWrapMirror::GetDOD(HGRenderer*, int index, HGRect)
//   0x470d00  HWrapMirror::GetROI(HGRenderer*, int index, HGRect)
//   0x470de0  HWrapMirror::setMaskRect(PCRect<double> const&)
//   0x470e30  HWrapMirror::setMatrix(PCMatrix44Tmpl<double> const&)
//   0x471000  HWrapMirror::setInvMatrix(PCMatrix44Tmpl<double> const&)
//
// EXTERNAL FUNCTIONS REFERENCED (undecoded — throwing stubs cite their addrs):
//   * HgcWrapMirror::~HgcWrapMirror() [D2]      @Ozone 0x6c4fc0
//     (tail-called by HWrapMirror::~HWrapMirror D1 @0x470c85; also by D0 @0x470c99)
//   * HGObject::operator delete(void*)           @Ozone __stubs 0x6def6a
//     (tail-jmp'd by HWrapMirror::~HWrapMirror D0 @0x470ca7 to free the object)
//   * HGRectIsNull(HGRect)                       @Ozone __stubs 0x6dcc9c
//     (called by GetDOD @0x470ccd on the input rect: rdi=x/y quad, rsi=w/h quad)
//   * HGRectMake4i(int, int, int, int)           @Ozone __stubs 0x6dcca8
//     (called by GetROI @0x470dba after computing floor/ceil integer corners)
//   * PCMatrix44Tmpl<double>::transformRect<double>(
//         PCRect<double> const&, PCRect<double>&) const
//                                                 @Ozone (Ozone-local; local
//                                                  callq at 0x470d37 target
//                                                  __ZNK14PCMatrix44TmplIdE13transformRectIdEEbRK6PCRectIT_ERS4_)
//   * Base-class vtable slot 0x60 (HGNode-inherited via HgcWrapMirror):
//        HgcWrapMirror::SetParameter(int idx, float, float, float, float)
//                                                 @Ozone 0x6c5010
//     Reached at call sites:
//        setMaskRect     @0x470e25 (idx=0)
//        setMatrix       @0x470f5a (idx=1), @0x470f8c (idx=2),
//                        @0x470fbe (idx=3), @0x470ff8 (idx=4)  [tail-jmpq]
//        setInvMatrix    @0x47112a (idx=5), @0x47115c (idx=6),
//                        @0x47118e (idx=7), @0x4711c8 (idx=8)  [tail-jmpq]
//
// DATA symbols read from Ozone __TEXT.__const (bit-exact bytes read
// through `lipo -thin x86_64 -output /tmp/OzoneX64` and unpacked as
// little-endian doubles):
//   _HGRectNull       @Ozone   16 bytes = { .x=0, .y=0, .right=0, .bottom=0 }
//                     (loaded by GetDOD @0x470cb4 as fall-through result;
//                      also @0x470cd9/0x470dc5 as the "not-null / on-fail"
//                      fallback in GetDOD / GetROI.)
//   _HGRectInfinite   @Ozone   16 bytes = { INT_MIN, INT_MIN, INT_MAX, INT_MAX }
//                     (loaded by GetDOD @0x470cd2 as the "input rect IS null"
//                      result — HGRectIsNull(rect) → true.)
//   const double pair @Ozone 0x7053c0  = [-1.0, -1.0]
//                     (movaps loaded into xmm0 by GetROI @0x470d1a — used as
...
//                     (movsd xmm4 @0x470d70; epsilon nudge added before each
//                      of the four floor/ceil roundsd operations.)
//
// STRUCT LAYOUT (recovered from setMaskRect/setMatrix/setInvMatrix
// stores; base HgcWrapMirror subobject not yet decoded here — it is a
// standard HGNode-derived compositor so occupies 0x000..≥0x1a0):
//   HWrapMirror {
//     +0x000..+0x19f  HgcWrapMirror subobject (vptr, HGNode base, plus
 ...
 *  Called from GetROI @0x470d37.  Body unported: the transform math
...
function PCMatrix44Tmpl_double__transformRect(
  ...
  throw new Error(
    "PCMatrix44Tmpl<double>::transformRect<double>(PCRect const&, PCRect&) const " +
      "@Ozone (call site 0x470d37) not yet transcribed",
  );
}

/** HgcWrapMirror::~HgcWrapMirror() [D2 base dtor] — @Ozone 0x6c4fc0.
 *  Tail-called by both HWrapMirror::~HWrapMirror overloads (D1 @0x470c85,
 *  D0 @0x470c99).  Body unported. */
function HgcWrapMirror_D2(_self: HWrapMirror): void {
  throw new Error(
    "HgcWrapMirror::~HgcWrapMirror() [D2] @Ozone 0x6c4fc0 not yet transcribed " +
      "(called from HWrapMirror dtors @0x470c85 / @0x470c99)",
  );
}

/** HGObject::operator delete(void*) — @Ozone __stubs 0x6def6a (resolves to
 *  the runtime allocator's `free`).  Tail-jmp'd by HWrapMirror::~HWrapMirror
 *  D0 @0x470ca7 after D2. */
function HGObject_operator_delete(_p: HWrapMirror): void {
  throw new Error(
    "HGObject::operator delete(void*) @Ozone stub 0x6def6a " +
      "not yet transcribed (called from HWrapMirror D0 @0x470ca7)",
  );
}

/** HGRectIsNull(HGRect) — @Ozone __stubs 0x6dcc9c.  Reads the 16-byte
 *  HGRect passed by value in (rdi, rsi) and returns bool in eax.
 *  Called from GetDOD @0x470ccd. */
function HGRectIsNull(_r: HGRect): boolean {
  throw new Error(
    "HGRectIsNull(HGRect) @Ozone stub 0x6dcc9c not yet transcribed " +
      "(called from HWrapMirror::GetDOD @0x470ccd)",
  );
}
...
/** Instance of HWrapMirror.  Full 0x000..0x1a0 base subobject is opaque
 *  and threaded through the base-class SetParameter vcall; only the
 *  HWrapMirror-added fields (maskRect / matrix / invMatrix) are named. */
export class HWrapMirror {
   ...
   *  HgcWrapMirror::SetParameter @Ozone 0x6c5010 (undecoded).  The four
  ...
  SetParameter(
    ...
    throw new Error(
      "HgcWrapMirror::SetParameter(int, float, float, float, float) @Ozone 0x6c5010 " +
        "not yet transcribed (base vtable slot 0x60 reached from HWrapMirror::" +
        "setMaskRect/setMatrix/setInvMatrix)",
    );
  }

  // -------------------------------------------------------------------------
  // ~HWrapMirror @0x470c80 [D1 complete dtor]  /  @0x470c90 [D0 deleting dtor]
  // -------------------------------------------------------------------------
  //
  // D1 @0x470c80:  pushq %rbp; movq %rsp, %rbp; popq %rbp;
  //                jmp __ZN13HgcWrapMirrorD2Ev
  //   → pure tail-jmp: run the base class D2 dtor on `this`.
  //
  // D0 @0x470c90:  pushq %rbp; movq %rsp, %rbp; pushq %rbx; pushq %rax;
  //                movq %rdi, %rbx;
  //                callq __ZN13HgcWrapMirrorD2Ev            @0x470c99
  //                movq %rbx, %rdi;
  //                addq $0x8, %rsp; popq %rbx; popq %rbp;
  //                jmp <__stub HGObject::operator delete>   @0x470ca7
  //   → run base D2 then tail-jmp HGObject::operator delete on `this`.

  /** HWrapMirror::~HWrapMirror() [D1 complete dtor] — @0x470c80.
   *  Tail-jmp to HgcWrapMirror::~HgcWrapMirror (D2).  Does NOT free `this`. */
  dtor_D1(): void {
    // @0x470c85: jmp __ZN13HgcWrapMirrorD2Ev
    HgcWrapMirror_D2(this);
  }

  /** HWrapMirror::~HWrapMirror() [D0 deleting dtor] — @0x470c90.
   *  Runs D2 then tail-jmps HGObject::operator delete on `this`. */
  dtor_D0(): void {
    // @0x470c99: callq __ZN13HgcWrapMirrorD2Ev
    HgcWrapMirror_D2(this);
    // @0x470ca7: jmp <__stubs HGObject::operator delete>
  ...
  //   callq <__stub HGRectIsNull>               @0x470ccd
  ...
  //   popq  %rbp; retq
  //
  // Semantics: if the caller asks for an output index other than 0, HWrapMirror
  // has no domain-of-definition on that port and returns the empty rect. On
  // port 0, if the *input* rect is null, the DOD is the infinite rect (the
  // wrap-mirror is defined everywhere on a degenerate input); otherwise the
  // DOD is HGRectNull.  (Note: the CMOV predicate is `cmove` = "move if
  // ZF==1"; here ZF=1 means eax==0, i.e. HGRectIsNull returned FALSE — the
  // rect is NOT null — so we take the &Infinite path in that case.  This
  // inversion is the actual x86 semantics; we mirror it directly below.)

  GetDOD(_renderer: HGRenderer, index: number, rect: HGRect): HGRect {
    // @0x470cb0-0x470cc2: index != 0 path.
    if ((index | 0) !== 0) {
      // returns _HGRectNull by value.
      return { x: HGRectNull.x, y: HGRectNull.y, right: HGRectNull.right, bottom: HGRectNull.bottom };
    }
    // @0x470cc3-0x470cf9: index == 0 path.
    const isNull = HGRectIsNull(rect); // @0x470ccd
  ...
  //   testl %edx,%edx                                 ; index == 0?
  //   jne   0x470dc5                                  ; else → return HGRectNull
  ...
  //   callq PCMatrix44Tmpl<double>::transformRect<double>  @0x470d37
  //           ( invMatrix,  maskRect,  &stackRect )   → bool eax
  //   testb %al,%al
  //   je    0x470dc5                                  ; on false → HGRectNull
  //
  ...
  //   callq <__stub HGRectMake4i>                   @0x470dba
  //   addq $0x20,%rsp; popq %rbp; retq
  //
  // 0x470dc5:  ; the "index != 0 OR transformRect failed" branch
  ...
  // ports, or if transformRect returns false, return HGRectNull.

  GetROI(_renderer: HGRenderer, index: number, _rect: HGRect): HGRect {
    // @0x470d08: index == 0?
    if ((index | 0) !== 0) {
      // @0x470dc5: return HGRectNull.
      return { x: HGRectNull.x, y: HGRectNull.y, right: HGRectNull.right, bottom: HGRectNull.bottom };
    }

    // @0x470d13-0x470d33: prepare a scratch PCRect on the stack.
    ...
    // @0x470d37: transformRect(this->invMatrix, this->maskRect, &stackRect).
  ...
  setMatrix(m: readonly number[]): void {
    // @0x470e3d-0x470e47: self-assignment guard.  In JS/TS we can't
    ...
    if (m !== (this.matrix as readonly number[])) {
      // @0x470e4d-0x470f23: 16× movsd copy (row-major, elt by elt).
      for (let i = 0; i < 16; i++) {
        this.matrix[i] = m[i];
      }
    }
  ...
  setInvMatrix(m: readonly number[]): void {
    // @0x47100d-0x471017: self-assignment guard against this->invMatrix.
    if (m !== (this.invMatrix as readonly number[])) {
      // @0x47101d-0x4710f3: 16× movsd copy.
      for (let i = 0; i < 16; i++) {
