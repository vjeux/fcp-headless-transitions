...
# TABLE_PROV — infra/parser DATA TABLES (scope/tag maps, factory tables) are decoded from PCScope
# descriptor arrays / PCSerializer, not from one function address. Their provenance is the
# generating tool + re/*.json source, or an explicit PCScope/PCSerializer/AUTO-GENERATED cite.
# Such files are GROUNDED without an @0xADDR (P1) and may carry their decoded id tables (P2).
TABLE_PROV  = re.compile(r"AUTO-GENERATED by|gen_[a-z_]+\.py|from re/[\w.]*\.json|PCScope|PCSerializer|createSceneNode|OZFactories", re.I)
...
def check(path):
    ...
    table_prov = bool(TABLE_PROV.search(text))
    grounded = (has_addr and bool(DECODE_REF.search(text))) or table_prov
    if exports and not has_addr and not table_prov:
