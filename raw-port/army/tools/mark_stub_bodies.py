...
def _throwonly_methods():
    ...
    for p in glob.glob(os.path.join(SRC,'**','*.ts'), recursive=True):
        fileclass=os.path.basename(p)[:-3]
        text=open(p,encoding='utf-8',errors='replace').read()
        for m in ss.DEF.finditer(text):
            name=m.group('mname')            # CLASS MEMBERS ONLY (free fns aren't ledger-1:1 reliably)
            if not name or m.group('fname'): continue
            if name in ('if','for','while','switch','catch','return','function','constructor'): continue
            if ss._structural_depth(text,m.start())!=1: continue
            if not ss._enclosing_brace_is_class(text,m.start()): continue
