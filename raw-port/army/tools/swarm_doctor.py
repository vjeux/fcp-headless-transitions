...
def check_layer_letters():
    ...
    # READ THE TABLE, and fall back to the old print form. prove_all's layers became
    # `LAYER2 = [(label, description, command, token), ...]` rows (#726), which removed the letter
    # collision this check was written for — the label is now data, so two PRs cannot be assigned
    # the same one by a merge. This check is still worth keeping (a duplicate row is possible by
    # hand), but its pattern went stale the moment the table landed, and it said so: `UNKNOWN —
    # found no LAYER labels ... the label format changed, or this check's pattern is stale; it is
    # not evidence of anything`. That is the check behaving correctly, and it is also exactly how a
    # guard quietly stops covering anything, so it is fixed here rather than left honest-but-inert.
    labels = re.findall(r'^\s*\(\s*"(\d+[a-z]?)",\s*$', src, re.M)
    if not labels:
