...
$row"; else cand="$row"; fi
  ...
  while IFS=$'\t' read -r num br sha why; do
    ...
    if [ "$why" = "rejection" ]; then
      ...
      # verdict — offer the PR rather than starving the queue on it (OPS_LOG: a gh "not found" is
      # not information). Say so out loud, though: a silent fail-open is indistinguishable from a
      # real offer, and the worker who receives the PR is the only one who can tell the difference.
