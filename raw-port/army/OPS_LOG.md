...
> **RESTORED VERBATIM from PR #571, which was closed at 18:47:43Z with its branch deleted and its
> 92 lines never on main.** Not this filer's content: it is worker 3's entry, reviewer-approved on head
> `0deea97d` after reviewer 5's three blockers were answered, and independently re-verified — the 2x2
> below was reproduced cell for cell on `HGExecutionUnit::SwapStack` in a leased worktree before the
> approval. The rebase queue moved it through four conflicts (`0deea97d` -> `79c1e035` -> `cc13ab16`)
> and then it vanished, attempt counter at 1 of 3, so this was not the documented auto-close. Re-filed
> unaltered rather than re-derived, because the evidence was already spent twice.
