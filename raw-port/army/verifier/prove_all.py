...
def layer2():
    ...
    # 2j — the AUTHOR-ANSWERED test in the rework queue. Its skip used to be a bare `rej != head`
    # SHA comparison, so a head moved by a reviewer's update-branch or by a conflict merge read as
    # "the author answered" and the PR left the rework queue with its reviewer's asks unaddressed
    # (#656, measured live). Pinned here because the direction of the error matters: not offering a
    # rejected PR strands finished review evidence where nothing else will notice, and the queue's
    # own comment says an over-offer costs one look. Real git fixtures, no network, no gh. ~1s.
    r10 = run(["bash", os.path.join(TOOLS, "test_rework_author_answered.sh")])
    ok10 = "test_rework_author_answered: PASS" in r10.stdout
    print("LAYER 2j (rework queue — a MOVED head is not by itself an answer):",
          "PASS" if ok10 else "FAIL")
    if not ok10: print(r10.stdout[-1200:], r10.stderr[-400:])
    # (2j because main ends at 2i today. #650 also carries a 2j; whichever lands second renumbers —
    #  the letter is a label, and two layers claiming one letter is a silent collision.)
