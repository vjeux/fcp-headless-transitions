#!/usr/bin/env python3
"""Differential oracle for HgcBT2100_HLG_InverseOETF::RenderTile_AVX(HGTile*)
   @Helium 0x3b1660  (__ZN25HgcBT2100_HLG_InverseOETF14RenderTile_AVXEP6HGTile)

    arch -x86_64 /usr/bin/python3 \\
        raw-port/re/oracle/HgcBT2100_HLG_InverseOETF_RenderTile_AVX_oracle.py
...
`_dyld_get_image_vmaddr_slide(Helium) + 0x3b1660` through ozone_loader.local_fn, and the opcode
...
raw-port/src/render/HgcBT2100_HLG_InverseOETF.ts, it does not restate it); every f32 of the destination
...
Three bank presets:
  * CTOR — the exact 26-slot bank the class ctor @0x3b1d20 (C2) builds (every constant read out of
...
SYM = "__ZN25HgcBT2100_HLG_InverseOETF14RenderTile_AVXEP6HGTile"
ADDR = 0x3B1660
...
TS_DRIVER = os.path.join(REPO, "raw-port", "re", "oracle",
                         "HgcBT2100_HLG_InverseOETF_RenderTile_AVX_driver.ts")

BANK_SIZE = 0x1A0          # 13 slots x 32 bytes (__Znam(0x1c7), aligned) == PARAM_BUF_LIVE_BYTES in the port
...
SLOTS = [(0x00, "s"), (0x04, "s"), (0x20, "s"), (0x24, "s"), (0x28, "s"), (0x2C, "s"),
         (0x40, "v"), (0x60, "v"), (0x80, "v"), (0xA0, "v"), (0xC0, "v"), (0xE0, "v"),
         (0x100, "v"), (0x120, "v"), (0x140, "i")]
...
CTOR_SLOTS = {
    2:  (0.0, 0.0, 0.0, 0.004999999888241291),      # @0x894e00  alphaThresh (alpha lane differs)
    3:  (-127.0, -127.0, -127.0, 1.0),              # @0x894e10  exp2LowClamp (alpha lane 1.0)
    4:  (0.0017952255439013243,) * 3 + (0.0,),      # @0x88e010  exp2 C5
    5:  (0.009189177304506302,) * 3 + (0.0,),       # @0x88e020  exp2 C4
    6:  (0.055661238729953766,) * 3 + (0.0,),       # @0x88e030  exp2 C3
    7:  (0.2402067929506302,) * 3 + (0.0,),         # @0x88e040  exp2 C2
    8:  (0.6931475400924683,) * 3 + (1.0,),         # @0x894e20  ln2 (alpha lane 1.0)
    9:  (1.0, 1.0, 1.0, 0.004999999888241291),      # @0x894e30  one (alpha lane 0.005)
    10: ("0x0000007f",) * 3 + ("0x00000000",),      # @0x88df70  the i32 exp2 bias
    11: ("0xffffffff",) * 3 + ("0x00000000",),      # @0x88c7f0  — not read by this kernel
    12: ("0x00000000",) * 3 + ("0xffffffff",),      # @0x85fc40  — not read by this kernel
}
...
def ctor_bank(p0, p1):
    """The bank exactly as the ctor leaves it, with the two settable slots filled. Each slot holds
    its float4 TWICE, so a 32-byte ymm load of a slot is a broadcast-ready (x,y,z,w,x,y,z,w)."""
    b = bytearray(b"\x00" * BANK_SIZE)
    for slot, lanes in CTOR_SLOTS.items():
        for half in (0x00, 0x10):
            for l in range(4):
                put_lane(b, slot * 0x20 + half + 4 * l, lanes[l])
    for slot, quad in ((0, p0), (1, p1)):
        for half in (0x00, 0x10):
            for l in range(4):
...
def realistic_bank():
    # HLG inverse-OETF shape: a quadratic segment below the breakpoint, exponential above.
    return ctor_bank(p0=(0.5, 0.3333333, 0.0, 0.0), p1=(8.359375, -2.2, 0.28466892, 0.0))


def special_bank():
    # Drives the gate the other way, so the corpus exercises BOTH arms of the final vblendvps.
    return ctor_bank(p0=(4.0, -1.5, 0.0, 0.0), p1=(0.5, 0.25, 2.0, -0.75))
