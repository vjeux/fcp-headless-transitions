...
function planeToHex(a: Float32Array): string[] {
  ...
  const out: string[] = [];
  for (let i = 0; i < a.length; i++) out.push(dv.getUint32(4 * i, true).toString(16).padStart(8, "0"));
  return out;
}
...
process.stdin.on("end", () => {
  ...
  const out = cases.map((c) => {
    ...
    // A non-zero shift is the harness-liveness mutant: it makes every State read come from the
