#!/usr/bin/env python3
"""Differential oracle for HgcBT2100_PQ_OOTF::RenderTile_AVX(HGTile*)
   @Helium 0x3a59d0  (__ZN17HgcBT2100_PQ_OOTF14RenderTile_AVXEP6HGTile)

    arch -x86_64 /usr/bin/python3 \\
        raw-port/re/oracle/HgcBT2100_PQ_OOTF_RenderTile_AVX_oracle.py
...
`_dyld_get_image_vmaddr_slide(Helium) + 0x3a59d0` through ozone_loader.local_fn, and the opcode
...
raw-port/src/render/HgcBT2100_PQ_OOTF.ts, it does not restate it); every f32 of the destination
...
Three bank presets:
  * CTOR — the exact 26-slot bank the class ctor @0x3a6a50 builds (every constant read out of
...
SYM = "__ZN17HgcBT2100_PQ_OOTF14RenderTile_AVXEP6HGTile"
ADDR = 0x3A59D0
...
TS_DRIVER = os.path.join(REPO, "raw-port", "re", "oracle",
                         "HgcBT2100_PQ_OOTF_RenderTile_AVX_driver.ts")

BANK_SIZE = 0x340          # 26 slots x 32 bytes, as the ctor allocates (__Znam(0x367), aligned)
...
SLOTS = [(0x00, "s"), (0x04, "s"), (0x08, "s"), (0x0C, "s"), (0x20, "s"), (0x24, "s"),
         (0x40, "v"), (0x60, "v"), (0x80, "v"), (0xA0, "v"), (0xC0, "v"), (0xE0, "v"),
         (0x100, "v"), (0x120, "v"), (0x140, "v"), (0x160, "v"), (0x180, "v"), (0x1A0, "v"),
         (0x1C0, "v"), (0x1E0, "v"), (0x200, "v"), (0x220, "v"), (0x240, "v"), (0x260, "v"),
         (0x280, "v"), (0x2A0, "v"), (0x2C0, "v"), (0x2E0, "i")]
...
CTOR_SLOTS = {
    3:  (1.0, 1.0, 1.0, 0.0),                       # @0x3ca9c0
    4:  ("0x807fffff",) * 3 + ("0x00000000",),      # @0x892090  mantissa|sign mask
    5:  ("0x00800000",) * 3 + ("0x00000000",),      # @0x858f70  +FLT_MIN
    6:  ("0x7f800000",) * 3 + ("0x00000000",),      # @0x88f440  +Inf
    7:  (127.0, 127.0, 127.0, 0.0),                 # @0x88ded0
    8:  (1.4142135381698608,) * 3 + (0.0,),         # @0x88dee0  sqrt(2)
    9:  (0.5, 0.5, 0.5, 0.0),                       # @0x85da90
    10: (0.2960891127586365,) * 3 + (0.0,),         # @0x88dfa0
    11: (-0.35917338728904724,) * 3 + (0.0,),       # @0x88dfb0
    12: (0.17290928959846497,) * 3 + (0.0,),        # @0x88dfc0
    13: (-0.27149274945259094,) * 3 + (0.0,),       # @0x88dfd0
    14: (0.4805939197540283,) * 3 + (0.0,),         # @0x88dfe0
    15: (-0.7213672399520874,) * 3 + (0.0,),        # @0x88dff0
    16: (1.4426966905593872,) * 3 + (0.0,),         # @0x88e000  1/ln2
    17: (-127.0, -127.0, -127.0, 0.0),              # @0x88df30
    18: (0.0017952255439013243,) * 3 + (0.0,),      # @0x88e010
    19: (0.009189177304506302,) * 3 + (0.0,),       # @0x88e020
    20: (0.055661238729953766,) * 3 + (0.0,),       # @0x88e030
    21: (0.2402067929506302,) * 3 + (0.0,),         # @0x88e040
    22: (0.6931475400924683,) * 3 + (0.0,),         # @0x88e050  ln2
    23: ("0x0000007f",) * 3 + ("0x00000000",),      # @0x88df70  the i32 exp2 bias
}
...
def ctor_bank(p0, p1, p2):
    """The bank exactly as the ctor @0x3a6a50 leaves it, with the three settable slots filled.
    ...
    for slot, quad in ((0, p0), (1, p1), (2, p2)):
        for half in (0x00, 0x10):
            for l in range(4):
                put_lane(b, slot * 0x20 + half + 4 * l, quad[l])
    return bytes(b)


def realistic_bank():
    # A BT.1886-shaped OOTF: out = (a·Y^g + b) above the breakpoint, c·Y below, then ^gamma.
    return ctor_bank(p0=(1.09929682, -0.09929682, 4.5, 0.018),
                     p1=(0.45, 2.4, 0.0, 0.0),
                     p2=(0.0, 0.0, 0.0, 0.0))


def special_bank():
    # Both exponents EQUAL slot 2, which is the only way to reach the two "substitute 1.0" arms.
    return ctor_bank(p0=(1.0, 0.0, 2.0, 0.5),
                     p1=(0.25, 0.25, 0.0, 0.0),
...
def main():
    ...
    banks = [realistic_bank(), special_bank(), adversarial_bank(rng), adversarial_bank(rng),
             ctor_bank(p0=(0.5, 0.25, -1.5, -0.5), p1=(3.0, 0.5, 0.0, 0.0),
                       p2=(0.1, -0.1, 0.0, 2.0))]
        ...
        for c in cases:
            ...
            for l in range(n):
                if kind == "i":
                    ...
                    # `cur * 1.5 + 0.125` is a NO-OP on an infinity or a NaN, and slot +0xc0 IS
                    # +Inf in the ctor's table — a probe that cannot move the value would report
                    # a dead slot that is merely un-perturbed. Substitute a distinct finite value
                    # there instead.
