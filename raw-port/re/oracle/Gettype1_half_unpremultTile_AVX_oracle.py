...
— say which, next to the numbers"):
  ...
  * CORPUS SENSITIVITY (not mutants): each State slot the kernel reads is perturbed in turn and
    ...
    for each of the four input lanes, which also pins the alpha-lane passthrough.
"""
import ctypes
import json
import os
...
# movl 0xc(%rdi),%eax ; subl 0x4(%rdi),%eax ; jle
BODY = bytes([0x8B, 0x47, 0x0C, 0x2B, 0x47, 0x04, 0x0F, 0x8E])
...
# The State slots this kernel reads, as (offset, kind) — 's' scalar, 'v' 32-byte vector,
...
def run_native(cases, fn):
    outs = []
    for c in cases:
        st_raw, st_addr = aligned_buffer(STATE_SIZE)
        ctypes.memmove(st_addr, c["state"], STATE_SIZE)
        in_raw, in_addr = aligned_buffer(4 * c["n"])
        out_raw, out_addr = aligned_buffer(4 * c["n"])
        view = (ctypes.c_char * (4 * c["n"])).from_address(in_addr)
        for i, v in enumerate(c["in"]):
            struct.pack_into("<f", view, 4 * i, v)
...
def diff_count(native_blobs, ts_hex):
    ...
    for c_i, (blob, th) in enumerate(zip(native_blobs, ts_hex)):
        for i, (a, b) in enumerate(zip(hexes(blob), th)):
            if a != b:
                bad += 1
                if first is None:
                    first = (c_i, i, a, b)
...
def main():
    ...
    for lane in range(4):
        ...
        note = ("  <- alpha: passed through untouched, so it MUST still move the output"
                if lane == 3 else "")
