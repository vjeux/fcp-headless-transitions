0x00000000000690 -- Hgc2FeatherOutside:
source_filename = "Hgc2FeatherOutside"
...
define <4 x float> @Hgc2FeatherOutside(<4 x float> %0, <4 x float> %1, <4 x float> %2, %struct._texture_2d_t addrspace(1)* %3, %struct._texture_2d_t addrspace(1)* %4, %struct._sampler_t addrspace(2)* nocapture readonly %5, %struct._sampler_t addrspace(2)* nocapture readonly %6, <4 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %7) local_unnamed_addr #0 {
  ...
  %13 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t addrspace(1)* nocapture readonly %3, %struct._sampler_t addrspace(2)* nocapture readonly %5, <2 x float> %12, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #3, !alias.scope !33, !noalias !30
  %14 = extractvalue { <4 x float>, i8 } %13, 0
  %15 = shufflevector <4 x float> %2, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %16 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t addrspace(1)* nocapture readonly %4, %struct._sampler_t addrspace(2)* nocapture readonly %6, <2 x float> %15, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #3, !alias.scope !33, !noalias !30
  %17 = extractvalue { <4 x float>, i8 } %16, 0
  %18 = fmul fast <4 x float> %9, <float poison, float poison, float poison, float 2.000000e+00>
  %19 = shufflevector <4 x float> %18, <4 x float> poison, <4 x i32> <i32 3, i32 3, i32 3, i32 3>
  %20 = fmul fast <4 x float> %19, %17
  %21 = fsub fast <4 x float> <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>, %14
  %22 = tail call fast <4 x float> @air.fast_pow.v4f32(<4 x float> %21, <4 x float> <float 2.000000e+00, float 2.000000e+00, float 2.000000e+00, float 2.000000e+00>) #4
  %23 = fmul fast <4 x float> %20, %22
  %24 = fmul fast <4 x float> %11, <float 5.000000e-01, float 5.000000e-01, float 5.000000e-01, float 5.000000e-01>
  %25 = fadd fast <4 x float> %24, <float 5.000000e-01, float 5.000000e-01, float 5.000000e-01, float 5.000000e-01>
  %26 = fsub fast <4 x float> %20, %23
  %27 = fmul fast <4 x float> %26, %25
  %28 = fadd fast <4 x float> %23, %14
  %29 = fadd fast <4 x float> %28, %27
  %30 = tail call fast <4 x float> @air.fast_fmin.v4f32(<4 x float> %29, <4 x float> <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>) #4
  ret <4 x float> %30
}
...
!15 = !{<4 x float> (<4 x float>, <4 x float>, <4 x float>, %struct._texture_2d_t addrspace(1)*, %struct._texture_2d_t addrspace(1)*, %struct._sampler_t addrspace(2)*, %struct._sampler_t addrspace(2)*, <4 x float> addrspace(2)*)* @Hgc2FeatherOutside, !16, !18}
!16 = !{!17}
...
!32 = distinct !{!32, !"air-alias-scopes(Hgc2FeatherOutside)"}
!33 = !{!34, !35}
