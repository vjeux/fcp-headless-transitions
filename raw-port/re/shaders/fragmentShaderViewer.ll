...
source_filename = "fragmentShaderViewer"
...
define <4 x float> @fragmentShaderViewer(<4 x float> %0, %struct._texture_2d_t addrspace(1)* %1) local_unnamed_addr #0 {
  %3 = extractelement <4 x float> %0, i64 0
  %4 = tail call i32 @air.get_width_texture_2d(%struct._texture_2d_t addrspace(1)* nocapture readonly %1, i32 0) #4, !alias.scope !22
  %5 = tail call fast float @air.convert.f.f32.u.i32(i32 %4) #5
  %6 = fdiv fast float %3, %5
  %7 = extractelement <4 x float> %0, i64 1
  %8 = tail call i32 @air.get_height_texture_2d(%struct._texture_2d_t addrspace(1)* nocapture readonly %1, i32 0) #4, !alias.scope !22
  %9 = tail call fast float @air.convert.f.f32.u.i32(i32 %8) #5
  %10 = fdiv fast float %7, %9
  %11 = fsub fast float 1.000000e+00, %10
  %12 = insertelement <2 x float> undef, float %6, i64 0
  %13 = insertelement <2 x float> %12, float %11, i64 1
  %14 = tail call { <4 x half>, i8 } @air.sample_texture_2d.v4f16(%struct._texture_2d_t addrspace(1)* nocapture readonly %1, %struct._sampler_t addrspace(2)* nocapture readonly bitcast ([2 x i64] addrspace(2)* @__air_sampler_state to %struct._sampler_t addrspace(2)*), <2 x float> %13, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #6
  %15 = extractvalue { <4 x half>, i8 } %14, 0
  %16 = tail call fast <4 x float> @air.convert.f.v4f32.f.v4f16(<4 x half> %15) #5
  ret <4 x float> %16
}
...
!15 = !{<4 x float> (<4 x float>, %struct._texture_2d_t addrspace(1)*)* @fragmentShaderViewer, !16, !18}
!16 = !{!17}
...
!20 = !{i32 1, !"air.texture", !"air.location_index", i32 0, i32 1, !"air.sample", !"air.arg_type_name", !"texture2d<half, sample>", !"air.arg_name", !"colorMap"}
!21 = !{!"air.sampler_state", [2 x i64] addrspace(2)* @__air_sampler_state}
!22 = !{!23}
!23 = distinct !{!23, !24, !"air-alias-scope-textures"}
!24 = distinct !{!24, !"air-alias-scopes(fragmentShaderViewer)"}
