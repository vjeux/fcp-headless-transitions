0x00000000002200 -- HgcColorGammaCorrectAlpha:
source_filename = "HgcColorGammaCorrectAlpha"
...
define <4 x float> @HgcColorGammaCorrectAlpha(<4 x float> %0, <4 x float> %1, %struct._texture_2d_t.2 addrspace(1)* nocapture readonly %2, %struct._sampler_t.3 addrspace(2)* nocapture readonly %3) local_unnamed_addr #0 {
  %5 = shufflevector <4 x float> %1, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %6 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t.2 addrspace(1)* nocapture readonly %2, %struct._sampler_t.3 addrspace(2)* nocapture readonly %3, <2 x float> %5, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #3, !alias.scope !23
  %7 = extractvalue { <4 x float>, i8 } %6, 0
  %8 = extractelement <4 x float> %7, i64 3
  %9 = tail call fast float @air.fast_pow.f32(float %8, float 0x3FE05C22A0000000) #4
...
!15 = !{<4 x float> (<4 x float>, <4 x float>, %struct._texture_2d_t.2 addrspace(1)*, %struct._sampler_t.3 addrspace(2)*)* @HgcColorGammaCorrectAlpha, !16, !18}
!16 = !{!17}
...
!25 = distinct !{!25, !"air-alias-scopes(HgcColorGammaCorrectAlpha)"}
