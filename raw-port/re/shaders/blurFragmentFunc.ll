0x00000000014ee9 -- blurFragmentFunc:
source_filename = "blurFragmentFunc"
...
define <4 x float> @blurFragmentFunc(<4 x float> %0, <2 x float> %1, %struct._texture_2d_t addrspace(1)* %2, <2 x float> addrspace(2)* nocapture noundef readonly align 8 dereferenceable(8) "air-buffer-no-alias" %3, i8 addrspace(2)* nocapture noundef readonly align 1 dereferenceable(1) "air-buffer-no-alias" %4) local_unnamed_addr #0 {
...
19:                                               ; preds = %19, %5
  %20 = phi <4 x float> [ zeroinitializer, %5 ], [ %36, %19 ]
  %21 = phi i32 [ 0, %5 ], [ %37, %19 ]
  %22 = add nsw i32 %21, -3
  ...
  %32 = fadd fast <2 x float> %26, %1
  %33 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t addrspace(1)* nocapture readonly %2, %struct._sampler_t addrspace(2)* nocapture readonly bitcast ([2 x i64] addrspace(2)* @__air_sampler_state.2 to %struct._sampler_t addrspace(2)*), <2 x float> %32, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #6
  %34 = extractvalue { <4 x float>, i8 } %33, 0
  %35 = fmul fast <4 x float> %31, %34
  %36 = fadd fast <4 x float> %35, %20
  %37 = add nuw nsw i32 %21, 1
  %38 = icmp eq i32 %37, 7
  br i1 %38, label %16, label %19, !llvm.loop !41
...
39:                                               ; preds = %16
  ...
  %42 = tail call fast <4 x float> @air.fast_fmax.v4f32(<4 x float> %36, <4 x float> %41) #5
  br label %43
...
declare <4 x float> @air.fast_fmax.v4f32(<4 x float>, <4 x float>) local_unnamed_addr #2
...
!15 = !{<4 x float> (<4 x float>, <2 x float>, %struct._texture_2d_t addrspace(1)*, <2 x float> addrspace(2)*, i8 addrspace(2)*)* @blurFragmentFunc, !16, !18}
!16 = !{!17}
...
!21 = !{i32 2, !"air.texture", !"air.location_index", i32 0, i32 1, !"air.sample", !"air.arg_type_name", !"texture2d<float, sample>", !"air.arg_name", !"tex"}
...
!30 = distinct !{!30, !"air-alias-scopes(blurFragmentFunc)"}
!31 = !{!32, !33}
