0x00000000009660 -- waveform_vertex_shader:
source_filename = "waveform_vertex_shader"
...
define <{ <4 x float>, <4 x float>, <2 x float>, float }> @waveform_vertex_shader(i32 noundef %0, <2 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %1, %struct.waveform_state_t addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %2, %struct._texture_2d_t addrspace(1)* nocapture readonly %3) local_unnamed_addr #0 {
  ...
  %12 = extractvalue %struct._rasterizer_data_t %11, 1
  %13 = getelementptr inbounds %struct.waveform_state_t, %struct.waveform_state_t addrspace(2)* %2, i64 0, i32 7
  %14 = load i32, i32 addrspace(2)* %13, align 16, !tbaa !36, !alias.scope !34, !noalias !35
...
15:                                               ; preds = %4
  %16 = getelementptr inbounds %struct.waveform_state_t, %struct.waveform_state_t addrspace(2)* %2, i64 0, i32 6
  %17 = load <4 x float>, <4 x float> addrspace(2)* %16, align 16, !tbaa !45, !alias.scope !34, !noalias !35
  br label %18

18:                                               ; preds = %15, %4, %4
  %19 = phi <4 x float> [ %17, %15 ], [ %12, %4 ], [ %12, %4 ]
  %20 = extractvalue %struct._rasterizer_data_t %11, 3
  %21 = extractvalue %struct._rasterizer_data_t %11, 2
  %22 = extractvalue %struct._rasterizer_data_t %11, 0
  %23 = getelementptr inbounds %struct.waveform_state_t, %struct.waveform_state_t addrspace(2)* %2, i64 0, i32 3
  %24 = load float, float addrspace(2)* %23, align 16, !tbaa !46, !alias.scope !34, !noalias !35
  %25 = tail call fast float @air.fast_clamp.f32(float %24, float 0.000000e+00, float 1.000000e+00) #6
  %26 = fmul fast float %25, 2.000000e+00
  %27 = fsub fast float 3.000000e+00, %26
  %28 = fmul fast float %25, %25
  %29 = fmul fast float %28, 0x3FD072B020000000
  %30 = fmul fast float %29, %27
  %31 = fadd fast float %30, 0x3FA0F27BC0000000
  %32 = insertelement <4 x float> %19, float %31, i64 3
  %33 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> undef, <4 x float> %22, 0
  %34 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %33, <4 x float> %32, 1
  %35 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %34, <2 x float> %21, 2
  %36 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %35, float %20, 3
  ret <{ <4 x float>, <4 x float>, <2 x float>, float }> %36
}
...
define internal fastcc %struct._rasterizer_data_t @_Z19waveform_rasterizerPU11MTLconstantK16waveform_state_tfDv4_Dh(%struct.waveform_state_t addrspace(2)* nocapture noundef readonly %0, float noundef %1, <4 x half> noundef %2) unnamed_addr #2 {
...
57:                                               ; preds = %45
  ...
  %60 = load i8, i8 addrspace(2)* %59, align 4, !tbaa !47, !range !48
  %61 = icmp eq i8 %60, 0
...
!15 = !{<{ <4 x float>, <4 x float>, <2 x float>, float }> (i32, <2 x float> addrspace(2)*, %struct.waveform_state_t addrspace(2)*, %struct._texture_2d_t addrspace(1)*)* @waveform_vertex_shader, !16, !21}
!16 = !{!17, !18, !19, !20}
...
!30 = distinct !{!30, !"air-alias-scopes(waveform_vertex_shader)"}
!31 = !{!32, !33}
