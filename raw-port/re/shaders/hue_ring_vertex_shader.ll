0x00000000003bb0 -- hue_ring_vertex_shader:
source_filename = "hue_ring_vertex_shader"
...
define <{ <4 x float>, <4 x float>, <2 x float>, float }> @hue_ring_vertex_shader(i32 noundef %0, <2 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %1, <4 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %2, %"struct.metal::matrix" addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %3) local_unnamed_addr #0 {
  ...
  %14 = getelementptr inbounds <4 x float>, <4 x float> addrspace(2)* %2, i64 %13
  %15 = load <4 x float>, <4 x float> addrspace(2)* %14, align 16, !tbaa !27, !alias.scope !39, !noalias !40
  %16 = getelementptr inbounds <2 x float>, <2 x float> addrspace(2)* %1, i64 %13
  %17 = load <2 x float>, <2 x float> addrspace(2)* %16, align 8, !tbaa !27, !alias.scope !41, !noalias !42
  %18 = shufflevector <2 x float> %17, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 undef, i32 undef>
  %19 = shufflevector <4 x float> %18, <4 x float> <float poison, float poison, float 0.000000e+00, float 1.000000e+00>, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
  %20 = tail call fast float @air.dot.v4f32(<4 x float> %19, <4 x float> %6) #2
  %21 = insertelement <4 x float> undef, float %20, i64 0
  %22 = tail call fast float @air.dot.v4f32(<4 x float> %19, <4 x float> %8) #2
  %23 = insertelement <4 x float> %21, float %22, i64 1
  %24 = tail call fast float @air.dot.v4f32(<4 x float> %19, <4 x float> %10) #2
  %25 = insertelement <4 x float> %23, float %24, i64 2
  %26 = tail call fast float @air.dot.v4f32(<4 x float> %19, <4 x float> %12) #2
  %27 = insertelement <4 x float> %25, float %26, i64 3
  %28 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> undef, <4 x float> %27, 0
  %29 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %28, <4 x float> %15, 1
...
!15 = !{<{ <4 x float>, <4 x float>, <2 x float>, float }> (i32, <2 x float> addrspace(2)*, <4 x float> addrspace(2)*, %"struct.metal::matrix" addrspace(2)*)* @hue_ring_vertex_shader, !16, !21}
!16 = !{!17, !18, !19, !20}
...
!24 = !{i32 2, !"air.buffer", !"air.location_index", i32 2, i32 1, !"air.read", !"air.address_space", i32 2, !"air.arg_type_size", i32 16, !"air.arg_type_align_size", i32 16, !"air.arg_type_name", !"float4", !"air.arg_name", !"Cs"}
...
!32 = distinct !{!32, !"air-alias-scopes(hue_ring_vertex_shader)"}
!33 = !{!34, !35}
