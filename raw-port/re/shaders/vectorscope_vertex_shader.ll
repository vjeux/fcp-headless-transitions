0x000000000063e0 -- vectorscope_vertex_shader:
source_filename = "vectorscope_vertex_shader"
...
define <{ <4 x float>, <4 x float>, <2 x float>, float }> @vectorscope_vertex_shader(i32 noundef %0, <2 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %1, %struct.vectorscope_state_t addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %2, %struct._texture_2d_t addrspace(1)* nocapture readonly %3) local_unnamed_addr #0 {
  ...
  %11 = extractvalue %struct._rasterizer_data_t %10, 1
  %12 = getelementptr inbounds %struct.vectorscope_state_t, %struct.vectorscope_state_t addrspace(2)* %2, i64 0, i32 5
  %13 = load i8, i8 addrspace(2)* %12, align 8, !tbaa !39, !range !45, !alias.scope !37, !noalias !38
  %14 = icmp eq i8 %13, 0
  %15 = select i1 %14, <4 x float> %11, <4 x float> <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float poison>
  %16 = extractvalue %struct._rasterizer_data_t %10, 2
  %17 = extractvalue %struct._rasterizer_data_t %10, 0
  %18 = getelementptr inbounds %struct.vectorscope_state_t, %struct.vectorscope_state_t addrspace(2)* %2, i64 0, i32 4
  %19 = load float, float addrspace(2)* %18, align 4, !tbaa !46, !alias.scope !37, !noalias !38
  %20 = tail call fast float @air.fast_clamp.f32(float %19, float 0.000000e+00, float 1.000000e+00) #6
  %21 = fmul fast float %20, 2.000000e+00
  %22 = fsub fast float 3.000000e+00, %21
  %23 = fmul fast float %20, %20
  %24 = fmul fast float %23, 0x3FD072B020000000
  %25 = fmul fast float %24, %22
  %26 = fadd fast float %25, 0x3FA0F27BC0000000
  %27 = insertelement <4 x float> %15, float %26, i64 3
  %28 = getelementptr inbounds %struct.vectorscope_state_t, %struct.vectorscope_state_t addrspace(2)* %2, i64 0, i32 3
  %29 = load float, float addrspace(2)* %28, align 16, !tbaa !47, !alias.scope !37, !noalias !38
  %30 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> undef, <4 x float> %17, 0
  %31 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %30, <4 x float> %27, 1
  %32 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %31, <2 x float> %16, 2
  %33 = insertvalue <{ <4 x float>, <4 x float>, <2 x float>, float }> %32, float %29, 3
  ret <{ <4 x float>, <4 x float>, <2 x float>, float }> %33
}
...
define internal fastcc %struct._rasterizer_data_t @_Z22vectorscope_rasterizerPU11MTLconstantK19vectorscope_state_tDv4_Dh(%struct.vectorscope_state_t addrspace(2)* nocapture noundef readonly %0, <4 x half> noundef %1) unnamed_addr #2 {
...
!15 = !{<{ <4 x float>, <4 x float>, <2 x float>, float }> (i32, <2 x float> addrspace(2)*, %struct.vectorscope_state_t addrspace(2)*, %struct._texture_2d_t addrspace(1)*)* @vectorscope_vertex_shader, !16, !21}
!16 = !{!17, !18, !19, !20}
...
!33 = distinct !{!33, !"air-alias-scopes(vectorscope_vertex_shader)"}
!34 = !{!35, !36}
