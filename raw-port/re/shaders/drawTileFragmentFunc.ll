0x000000000115e9 -- drawTileFragmentFunc:
source_filename = "drawTileFragmentFunc"
...
@__air_sampler_state.1 = internal addrspace(2) constant [2 x i64] [i64 34901797601036873, i64 0], align 8

; Function Attrs: argmemonly convergent mustprogress nofree nounwind readonly willreturn
define <4 x float> @drawTileFragmentFunc(<4 x float> %0, <2 x float> %1, %struct._texture_2d_t addrspace(1)* nocapture readonly %2) local_unnamed_addr #0 {
  %4 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t addrspace(1)* nocapture readonly %2, %struct._sampler_t addrspace(2)* nocapture readonly bitcast ([2 x i64] addrspace(2)* @__air_sampler_state.1 to %struct._sampler_t addrspace(2)*), <2 x float> %1, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #2
...
!15 = !{<4 x float> (<4 x float>, <2 x float>, %struct._texture_2d_t addrspace(1)*)* @drawTileFragmentFunc, !16, !18}
!16 = !{!17}
...
!22 = !{!"air.sampler_state", [2 x i64] addrspace(2)* @__air_sampler_state.1}
