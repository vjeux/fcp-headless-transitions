0x00000000002930 -- Hgc2ErodeOutside:
source_filename = "Hgc2ErodeOutside"
...
define <4 x float> @Hgc2ErodeOutside(<4 x float> %0, <4 x float> %1, <4 x float> %2, %struct._texture_2d_t.2 addrspace(1)* nocapture readnone %3, %struct._texture_2d_t.2 addrspace(1)* nocapture readonly %4, %struct._sampler_t.3 addrspace(2)* nocapture readnone %5, %struct._sampler_t.3 addrspace(2)* nocapture readonly %6, <4 x float> addrspace(2)* nocapture noundef readonly "air-buffer-no-alias" %7) local_unnamed_addr #0 {
  %9 = load <4 x float>, <4 x float> addrspace(2)* %7, align 16, !tbaa !27, !alias.scope !30, !noalias !33
  %10 = shufflevector <4 x float> %2, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %11 = tail call { <4 x float>, i8 } @air.sample_texture_2d.v4f32(%struct._texture_2d_t.2 addrspace(1)* nocapture readonly %4, %struct._sampler_t.3 addrspace(2)* nocapture readonly %6, <2 x float> %10, i1 true, <2 x i32> zeroinitializer, i1 false, float 0.000000e+00, float 0.000000e+00, i32 0) #3, !alias.scope !33, !noalias !30
  %12 = extractvalue { <4 x float>, i8 } %11, 0
  %13 = extractelement <4 x float> %9, i64 3
  %14 = fmul fast float %13, 0x3F947AE140000000
  %15 = fadd fast float %14, 0x3FEE666660000000
  %16 = tail call fast float @air.fast_fmax.f32(float %15, float 1.000000e+00) #4
  %17 = tail call fast float @air.fast_fmin.f32(float %16, float 2.000000e+00) #4
  %18 = insertelement <4 x float> poison, float %17, i64 0
  %19 = shufflevector <4 x float> %18, <4 x float> poison, <4 x i32> zeroinitializer
  %20 = fmul fast <4 x float> %19, %12
  %21 = tail call fast <4 x float> @air.fast_fmin.v4f32(<4 x float> %20, <4 x float> <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>) #4
  ret <4 x float> %21
}
...
!15 = !{<4 x float> (<4 x float>, <4 x float>, <4 x float>, %struct._texture_2d_t.2 addrspace(1)*, %struct._texture_2d_t.2 addrspace(1)*, %struct._sampler_t.3 addrspace(2)*, %struct._sampler_t.3 addrspace(2)*, <4 x float> addrspace(2)*)* @Hgc2ErodeOutside, !16, !18}
!16 = !{!17}
...
!20 = !{i32 1, !"air.fragment_input", !"user(texcoord0)", !"air.center", !"air.perspective", !"air.arg_type_name", !"float4", !"air.arg_name", !"texcoord0", !"air.arg_unused"}
!21 = !{i32 2, !"air.fragment_input", !"user(texcoord1)", !"air.center", !"air.perspective", !"air.arg_type_name", !"float4", !"air.arg_name", !"texcoord1"}
!22 = !{i32 3, !"air.texture", !"air.location_index", i32 0, i32 1, !"air.sample", !"air.arg_type_name", !"texture2d<float, sample>", !"air.arg_name", !"texture0", !"air.arg_unused"}
...
!24 = !{i32 5, !"air.sampler", !"air.location_index", i32 0, i32 1, !"air.arg_type_name", !"sampler", !"air.arg_name", !"sampler0", !"air.arg_unused"}
...
!32 = distinct !{!32, !"air-alias-scopes(Hgc2ErodeOutside)"}
!33 = !{!34, !35}
